...
function waitForStreamRegistration(id) {
  return new Promise((resolve) => {
    ...
    const poll = () => {
      ...
      if (Date.now() - startedAt >= PENDING_REGISTER_GRACE_MS)
...
// Generation completions surfaced to the user from either side:
//  - the page (toast/in-app) records handled ids via CHAT_RESPONSE_HANDLED
//  - this worker records ids it showed a push notification for
// Both sides consult the record so a completion never notifies twice (once
...
self.addEventListener("push", (event) => {
  ...
  if (payload?.type !== "chat-response" || !payload.body) return;
  event.waitUntil(
    (async () => {
      // The page may have already surfaced this completion before being
      // suspended; it records handled generation ids via postMessage and
      ...
      await self.registration.showNotification(payload.title || "RisuAI", {
        body: payload.body,
        tag: payload.generationId
          ? `chat-response:${payload.generationId}`
          : "chat-response",
        renotify: true,
  ...
  event.waitUntil(
    (async () => {
      ...
      for (const client of clients) {
        ...
        if (client.focus) await client.focus().catch(() => {});
        return;
      }
  ...
  if (event.data?.type === "REGISTER_STREAM_DOWNLOAD") {
    ...
    if (streamPort && id) {
      const readable = new ReadableStream({
        ...
        cancel() {
          try {
            streamPort.postMessage({ cancel: true });
          } catch {}
          try {
            streamPort.close();
          } catch {}
      ...
      switch (path[2]) {
          ...
          event.respondWith(
            (async () => {
              ...
              if (!(await waitForStreamRegistration(id))) {
                return new Response("Download stream not found or expired", {
                  status: 404,
                });
              }
        ...
        case "register": {
          ...
          const customContentType =
            headers.get("content-type") || headers.get("x-content-type");
          event.respondWith(
            registerCache(
              targetUrl,
