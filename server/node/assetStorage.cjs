...
const {
  ASSET_STORAGE_TYPES,
} = require("../../packages/protocol/storageConfig.cjs");
...
function loadS3Sdk() {
  ...
  const s3Require = require("module").createRequire(
...
class LocalFsStorage {
  ...
  async readThumbnail(hexPath, options = {}) {
    ...
    try {
      ...
      const thumbBuffer = await createThumbnailBuffer(
        originalBuffer,
        width,
        height,
      );
  ...
  async writeFromPath(hexPath, sourcePath) {
    ...
    if (isImageKey(key)) {
      fs.promises
        .readFile(fullPath)
        .then((data) => createThumbnailBuffer(data, 128, 128))
        .then(async (thumbBuffer) => {
          if (thumbBuffer) {
            if (!fs.existsSync(thumbDir)) {
              fs.mkdirSync(thumbDir, { recursive: true });
...
function loadMssql() {
  try {
    ...
    throw new Error(
      `MSSQL support requires the 'mssql' npm package to be installed: ${err.message}`,
    );
  }
}
...
class AzureSqlAssetStorage {
  ...
  async generateMissingThumbnails(onProgress) {
    ...
    const existingThumbs = new Set(
      (thumbRes.recordset || []).map((r) => r.asset_key),
    );
