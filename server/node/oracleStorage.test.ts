...
describe("Oracle empty-string row restoration", () => {
  it("restores the sentinel to an empty string", () => {
    ...
    expect(restoreEmptyStringInRow(row)).toEqual({
      name: "",
      note: "",
      other: "kept",
    });
...
describe("Oracle connection wrapper", () => {
  ...
  it("normalizes binds passed to execute", async () => {
    ...
    await wrapped.execute("INSERT INTO t (a) VALUES (:1)", ["", "x"], {
      autoCommit: false,
    });
  ...
  it("normalizes binds passed to executeMany", async () => {
    const conn = fakeConnection();
    const wrapped = wrapConnectionForEmptyStrings(conn);
    await wrapped.executeMany("INSERT INTO t (a, b) VALUES (:1, :2)", [
      ["", 1],
      ["x", ""],
    ]);
    expect(conn.executeMany).toHaveBeenCalledWith(
      ...
      [
        [ORACLE_EMPTY_STRING_SENTINEL, 1],
        ["x", ORACLE_EMPTY_STRING_SENTINEL],
      ],
    );
  });
...
describe("sync round-trip with empty string setting values", () => {
  ...
  it("round-trips an empty string setting value through sentinel storage", () => {
    ...
    const stored = (
      normalizeEmptyStringBinds(buildSettingBinds(values)) as unknown[][]
    ).map((binds) =>
      Object.fromEntries([
        ["setting_key", binds[0]],
        ["node_id", binds[1]],
        ["parent_node_id", binds[2]],
        ["member_key", binds[3]],
        ["encoded_member_key", binds[4]],
        ["position", binds[5]],
        ["value_type", binds[6]],
        ["text_value", binds[7]],
        ["encoded_text_value", binds[8]],
        ["number_value", binds[9]],
        ["boolean_value", binds[10]],
      ]),
    );
  ...
  it("round-trips an empty string object key through member_key", () => {
    const { values } = splitSetting("pluginCustomStorage", {
      "": "stored-value",
    });
    ...
    const binds = normalizeEmptyStringBinds(
      buildSettingBinds(values),
    ) as unknown[][];
  ...
  it("normalizes empty strings in named bind objects and arrays of objects", () => {
    ...
    const normalizedSingle = normalizeEmptyStringBinds(singleObj) as Record<
      string,
      any
    >;
    ...
    const arrayOfObjs = [
      { id: "char1", first_message: "" },
      { id: "char2", first_message: "hi" },
    ];
    const normalizedArray = normalizeEmptyStringBinds(arrayOfObjs) as Record<
      string,
      any
    >[];
