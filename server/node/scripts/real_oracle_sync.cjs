...
const {
  ...
  splitChat,
  splitMessage,
} = require("../storage/postgres/postgresRelationalCodec.cjs");
...
async function syncBackupToOracle(backupPath, options = {}) {
  ...
  if (!config.user || !config.password || !config.tnsAlias) {
    throw new Error(
      "Oracle DB credentials not found. Please set ORACLE_USER, ORACLE_PASSWORD, ORACLE_TNS_ALIAS or configure .env.oracle",
    );
  }
  ...
  console.log(
    `[Oracle Sync] Archive parsed (${sizeMb} MB, ${entryCount} total entries).`,
  );
  ...
  console.log(
    `[Oracle Sync] Payload summary: ${allChars.length} characters, ${allChatsForSync.length} chats, ${allMessagesForSync.length} messages, ${rootUpserts.length} settings`,
  );
  ...
  console.log(
    "[Oracle Sync] Executing storage.sync() against Oracle Database...",
  );
  ...
  const onProgress = (info) => {
    ...
    if (info.stage === "bulk_insert") {
      ...
      process.stdout.write(
        `\r⏳ [${elapsed}s] ${info.table}: ${info.current} / ${info.total} (${pct}%)        `,
      );
  ...
  console.log(
    `\n🎉 Real Oracle sync SUCCESS! Result:`,
    result,
    `(took ${duration}s)\n`,
  );
  ...
  if (options.shallowVerify !== false) {
    ...
    console.log(
      `[Oracle Sync] Verification succeeded! Loaded characters: ${loaded.characters?.length}, Revision: ${loaded.revision}`,
    );
  }
...
async function main() {
  ...
  if (!targetFile) {
    printUsage();
    console.error(
      "Error: Please specify the path to a .bin backup archive file.\n",
    );
    process.exit(1);
  }
