...
async function test() {
  ...
  if (!targetFile) {
    console.error(
      "Usage: node server/node/scratch_diag_sync.cjs <backup-file.bin>",
    );
    process.exit(1);
  }
      ...
      for (let j = 0; j < char.chats.length; j++) {
        if (char.chats[j].message) {
          for (let k = 0; k < char.chats[j].message.length; k++) {
            splitMessages.push(
              splitMessage({
                id: char.chats[j].message[k].id || `m-${k}`,
                chatId: char.chats[j].id,
                position: k,
                data: char.chats[j].message[k],
              }),
            );
          }
        }
  ...
  try {
    ...
    await storage._bulkInsertRows(
      ...
      "chat_messages",
      messageColumns,
      splitMessages.slice(0, 500).map((m) => m.core),
    );
