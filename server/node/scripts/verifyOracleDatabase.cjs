...
async function verifyOracleDatabase(options = {}) {
  ...
  try {
    ...
    for (const table of tables) {
      try {
        ...
        console.log(
          `  • ${table.padEnd(30)}: ${String(count).padStart(8)} rows`,
        );
      } catch (e) {
        console.log(
          `  • ${table.padEnd(30)}: (Table not accessible / ${e.message})`,
        );
      }
    }

    try {
      const sampleChars = await conn.execute(
        `SELECT id, name FROM character_characters FETCH FIRST 5 ROWS ONLY`,
      );
      console.log("\nSample Characters:");
  ...
  if (options.shallowVerify !== false) {
    ...
    console.log(
      `✅ loadStartupData verified! Characters count: ${loaded.characters?.length}, Revision: ${loaded.revision}`,
    );
  }
...
if (require.main === module) {
  ...
  if (opts.help) {
    console.log(`
Usage:
  node server/node/scripts/verifyOracleDatabase.cjs [options]
