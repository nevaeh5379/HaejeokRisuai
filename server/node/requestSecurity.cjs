...
function isSecurePostgresConfigRequest(req) {
    ...
    if (firstHeaderValue(headers['x-forwarded-ssl']).toLowerCase() === 'on') return true;
    if (firstHeaderValue(headers['front-end-https']).toLowerCase() === 'on') return true;
    if (firstHeaderValue(headers['x-url-scheme']).toLowerCase() === 'https') return true;
    if (firstHeaderValue(headers['cf-visitor']).includes('"scheme":"https"')) return true;
