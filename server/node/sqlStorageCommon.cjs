...
class SqlStorageBase {
  ...
  invalidateBootstrapCache(changedKeys) {
    if (
      ...
      !changedKeys.some((key) => BOOTSTRAP_SETTING_KEYS.includes(key))
    )
      return;
  ...
  async loadBootstrapData() {
    ...
    const pending = this.loadSettingKeys(BOOTSTRAP_SETTING_KEYS)
      .then(({ settings, hash }) => {
...
function createSqlStorageHelpers({
  ...
  const coldStorageKeyPattern = allowShortColdStorageKeys
    ? COMPAT_UUID_PATTERN
    : UUID_PATTERN;
  const coldStoragePathPattern = new RegExp(
  ...
  function validateColdStorageKeys(value, field = "keys") {
    ...
    return Array.from(
      new Set(keys.map((key) => normalizeColdStorageKey(key, `${field}[]`))),
    );
  }
  ...
  const validateProtocolSyncPayload = createSqlCommitValidator({
    PayloadError,
    maxIdLength,
  });
