...
function describeSqlCommitChange(payload) {
  ...
  const addCharacter = (id) => {
    if (typeof id === "string" && id) characterIds.add(id);
  };
  ...
  for (const row of payload?.chatManifests ?? [])
    addCharacter(row?.characterId);

  const rootUpsertKeys = (payload?.root?.upserts ?? [])
    .map((entry) => entry?.key)
    .filter((key) => typeof key === "string" && key);
  const rootDeleteKeys = (payload?.root?.deletes ?? []).filter(
...
function createRealtimeEventHub({
  heartbeatMs = 15_000,
  historyLimit = 512,
  generationMaxAgeMs = 60 * 60 * 1000,
} = {}) {
  ...
  function updateGenerationState(input, sourceClientId) {
    ...
    if (
      ...
      !["started", "finished", "failed", "aborted"].includes(state)
    )
      return null;
