...
function encodeRisuDb(dbObject: any): Uint8Array {
    ...
    const result = new Uint8Array(magicCompressedHeader.length + compressed.length)
...
function createMockArchiveBuffer(entries: { name: string; data: Uint8Array }[]): Buffer {
...
function parseBackupArchiveFromBuffer(buf: Buffer): { dbData: Buffer; entryCount: number } {
    let offset = 0
...
describe('Oracle Backup Archive Sync Test', () => {
    it('creates, decodes, and validates full backup archive sync with OracleStorage', async () => {
        ...
        const splitCharacters = decodedDb.characters.map((char: any, i: number) =>
            splitCharacter({ id: char.chaId || `char-${i}`, position: i, data: char })
        )
        ...
        expect(executedCalls[0].sql.toUpperCase()).toContain('INSERT INTO CHARACTER_ATTRIBUTES')
        ...
        for (const char of decodedDb.characters) {
            for (let j = 0; j < char.chats.length; j++) {
                ...
                allChatsForSync.push({ id: c.id, characterId: char.chaId, position: j, data: c })
                for (let k = 0; k < c.message.length; k++) {
                    allMessagesForSync.push({ id: c.message[k].id, chatId: c.id, position: k, data: c.message[k] })
                }
            }
        }
        ...
        const allSettingValueRows = splitSettingRows.flatMap((s: any) => s.values.map((v: any) => ({
        ...
        const mockLoadConn = {
            execute: vi.fn(async (sql: string) => {
                if (sql.includes('SELECT revision, initialized FROM system_storage_meta')) {
                ...
                if (sql.includes('FROM character_characters')) {
                    return { rows: [{ ID: 'char-test-1', NAME: 'Test Character', POSITION: 0, UPDATED_AT: Date.now(), IMAGE: '' }] }
                }
                if (sql.includes('FROM chat_chats')) {
                    return { rows: [{ ID: 'chat-test-1', CHARACTER_ID: 'char-test-1', POSITION: 0, UPDATED_AT: Date.now(), NAME: 'First Chat Session' }] }
                }
                return { rows: [] }
            }),
            rollback: vi.fn(async () => {}),
