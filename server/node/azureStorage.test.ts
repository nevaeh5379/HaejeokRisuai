...
describe("AzureStorage Driver & StorageDriver Integration", () => {
  ...
  it("validates SQL identifier safety", () => {
    ...
    expect(() =>
      assertSqlIdentifier("system.settings; DROP TABLE users;"),
    ).toThrow();
  });
  ...
  it("deletes only explicitly named characters, chats, and messages", async () => {
    ...
    class RecordingRequest {
      ...
      async query(q: string) {
        ...
        queries.push(t);
        if (
          /SELECT revision, initialized FROM \[system\]\.\[storage_meta\]/i.test(
            q,
          )
        ) {
      ...
      async batch(q: string) {
        queries.push(q.trim().replace(/\s+/g, " "));
        return { recordset: [] };
      }
    }
    class MockTx {
    ...
    const payload = {
      ...
      messages: [],
      messageManifests: [{ chatId: "chat-1", ids: ["msg-1"] }],
    ...
    expect(joined).toMatch(
      /DELETE target FROM \[character\]\.\[characters\] target INNER JOIN OPENJSON\(@character_delete_ids\)/,
    );
    expect(joined).not.toMatch(
      /DELETE FROM \[character\]\.\[characters\] WHERE id NOT IN/,
    );

    expect(joined).toMatch(
      /DELETE target FROM \[chat\]\.\[chats\] target INNER JOIN OPENJSON\(@chat_delete_ids\)/,
    );
    expect(joined).toMatch(
      /DELETE FROM \[chat\]\.\[messages\] WHERE chat_id = @msg_del_chat_id AND id IN \('msg-old'\)/,
    );
