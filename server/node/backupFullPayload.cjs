...
function buildFullBackupPayload(database) {
    ...
    const sourceCharacters = Array.isArray(database.characters) ? database.characters : [];
    for (let characterPosition = 0; characterPosition < sourceCharacters.length; characterPosition++) {
        ...
        const { chats: _chats, chaId: _chaId, detailsLoaded: _detailsLoaded, ...characterData } = character;
        characters.push({ id: characterId, position: characterPosition, data: characterData });
        const sourceChats = Array.isArray(character.chats) ? character.chats : [];
        const chatIds = [];
        for (let chatPosition = 0; chatPosition < sourceChats.length; chatPosition++) {
            const chat = sourceChats[chatPosition];
            const chatId = chat.id || crypto.randomUUID();
            chat.id = chatId;
            chatIds.push(chatId);
            const { message: _message, id: _id, messagesLoaded: _messagesLoaded, detailsLoaded: _chatDetailsLoaded, ...chatData } = chat;
            chats.push({ id: chatId, characterId, position: chatPosition, data: chatData });
            const messageIds = [];
            if (chat.messagesLoaded !== false) {
                const sourceMessages = Array.isArray(chat.message) ? chat.message : [];
                for (let messagePosition = 0; messagePosition < sourceMessages.length; messagePosition++) {
                    ...
                    messages.push({ id: messageId, chatId: chatId, position: messagePosition, data: messageData });
                }
            }
