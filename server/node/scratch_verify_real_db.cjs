...
async function verify() {
  ...
  try {
    const r1 = await conn.execute(
      `SELECT COUNT(*) AS CNT FROM character_characters`,
    );
    ...
    const r4 = await conn.execute(
      `SELECT COUNT(*) AS CNT FROM chat_message_prompt_toggles`,
    );
    console.log("Message Prompt Toggles in DB:", r4.rows[0][0]);

    const r5 = await conn.execute(
      `SELECT COUNT(*) AS CNT FROM system_setting_values`,
    );
  ...
  console.log(
    "loadStartupData successful! Characters:",
