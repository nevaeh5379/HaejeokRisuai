...
function parseBackupArchive(filePath) {
  ...
  if (!dbData) {
    throw new Error(
      `database.risudat not found inside archive: ${resolvedPath}`,
    );
  }
...
async function syncBackupToAzure(backupPath, options = {}) {
  ...
  if (!config.server || !config.database || !config.user || !config.password) {
    throw new Error(
      "Azure SQL DB credentials not found. Please set AZURE_HOST, AZURE_DATABASE, AZURE_USERNAME, AZURE_PASSWORD or configure .env.azure",
    );
  }
  ...
  console.log(
    `[Azure Sync] Archive parsed (${sizeMb} MB, ${entryCount} total entries).`,
  );
  ...
  console.log(
    `[Azure Sync] Payload summary: ${allChars.length} characters, ${allChatsForSync.length} chats, ${allMessagesForSync.length} messages, ${rootUpserts.length} settings`,
  );
  ...
  if (options.dryRun) {
    ...
    return {
      ...
      chats: allChatsForSync.length,
      messages: allMessagesForSync.length,
    };
  }
  ...
  console.log(
    "[Azure Sync] Executing storage.sync() against Azure SQL Database...",
  );
  ...
  console.log(
    `\n🎉 Real Azure SQL sync SUCCESS! Result:`,
    result,
    `(took ${duration}s)\n`,
  );
  ...
  if (options.shallowVerify !== false) {
    ...
    console.log(
      `[Azure Sync] Startup verification succeeded! Loaded characters: ${loaded.characters?.length}, Revision: ${loaded.revision}`,
    );
  }

  if (options.fullVerify) {
    console.log(
      "[Azure Sync] Testing exportDatabaseSnapshot() with messages from Azure SQL DB...",
    );
    ...
    console.log(
      `[Azure Sync] Full verification succeeded! Characters: ${fullLoaded.database?.characters?.length}, Chats: ${totalLoadedChats}, Messages: ${totalLoadedMessages}`,
    );
  }
...
`);
  ...
  if (!backupFile) {
    console.error(
      "Error: No backup .bin file found. Specify path via argument: node real_azure_sync.cjs Binary1.bin",
    );
    process.exit(1);
  }
