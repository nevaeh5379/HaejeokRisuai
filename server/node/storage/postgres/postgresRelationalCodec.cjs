...
function containsNul(value, seen = new Set()) {
  ...
  return Object.entries(value).some(
    ([key, item]) => key.includes("\0") || containsNul(item, seen),
  );
}
...
function splitCharacter(row) {
  ...
  const tags = relationalArray(source, "tags", attributes).map(
    (tag, position) => ({ character_id: row.id, position, tag }),
  );
  const greetings = [
  ...
  const biases = relationalArray(source, "bias", attributes).map(
    ([phrase, bias], position) => ({
      character_id: row.id,
      position,
      phrase,
      bias,
  ...
  const emotions = relationalArray(source, "emotionImages", attributes).map(
    ([emotion, asset], position) => ({
      character_id: row.id,
      position,
      emotion,
      asset,
  ...
  const modules = relationalArray(source, "modules", attributes).map(
    (moduleId, position) => ({
      character_id: row.id,
      position,
      module_id: moduleId,
  ...
  const chatFolders = relationalArray(source, "chatFolders", attributes).map(
    (folder, position) => ({
      character_id: row.id,
      position,
      folder_id: folder.id,
  ...
  const sdData = relationalArray(source, "sdData", attributes).map(
    ([key, value], position) => ({
      character_id: row.id,
      position,
      key,
...
function splitChat(row) {
  ...
  core.name ??= "";
  core.note ??= "";
  const suggestions = relationalArray(
    source,
    "suggestMessages",
    attributes,
  ...
  const bookmarks = relationalArray(source, "bookmarks", attributes).map(
    (messageId, position) => ({
      chat_id: row.id,
      position,
...
function splitMessage(row) {
  ...
  const generationInfo = containsNul(source.generationInfo)
    ? null
...
function rebuildCharacter(row, related = {}, options = {}) {
  ...
  const scalarMap = options.shallow
  ...
  if (row.kind === "group") {
    ...
    character.characterTalks = (related.groupMembers || []).map((item) =>
  ...
  if (!options.shallow) {
    ...
    character.alternateGreetings = (related.greetings || [])
      .filter((item) => item.greeting_type === "alternate")
      .map((item) => item.content);
    if (
      (related.greetings || []).some(
        (item) => item.greeting_type === "group-only",
      )
    ) {
      character.group_only_greetings = related.greetings
        .filter((item) => item.greeting_type === "group-only")
        .map((item) => item.content);
    }
    character.bias = (related.biases || []).map((item) => [
      item.phrase,
      Number(item.bias),
    ]);
    character.emotionImages = (related.emotions || []).map((item) => [
      item.emotion,
      item.asset,
    ]);
    character.modules = (related.modules || []).map((item) => item.module_id);
    character.customscript = (related.scripts || [])
      .filter((item) => item.script_kind === "custom")
      .map((item) => ({
    ...
    character.triggerscript = (related.scripts || [])
      .filter((item) => item.script_kind === "trigger")
      .map((item) => decodeJson(item.trigger_payload));
    character.sdData = (related.sdData || []).map((item) => [
      item.key,
      item.value,
    ]);
    const additionalAssets = (related.assets || [])
...
function rebuildChat(row, related = {}, options = {}) {
  ...
  if (!options.shallow) {
    ...
    if ((related.suggestions || []).length)
      chat.suggestMessages = related.suggestions.map((item) => item.content);
    if ((related.modules || []).length)
      chat.modules = related.modules.map((item) => item.module_id);
...
function rebuildMessage(row, related = {}) {
  ...
  if (related.promptInfo) {
    ...
    if ((related.promptToggles || []).length)
      message.promptInfo.promptToggles = related.promptToggles.map((item) => ({
        key: item.toggle_key,
        value: item.toggle_value,
      }));
    if ((related.promptItems || []).length)
      message.promptInfo.promptText = related.promptItems.map((item) =>
        decodeJson(item.payload),
      );
  }
