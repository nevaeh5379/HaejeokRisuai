...
function readSource(vendor: string, name: string): string {
  return fs.readFileSync(
    path.join(root, "server/node/storage", vendor, name),
    "utf8",
  );
}
...
describe("persistent branch schema parity", () => {
  it("defines branch graph tables for PostgreSQL", () => {
    const sql = readSource("postgres", "postgres-schema.sql");
  ...
  it("defines branch graph tables for Azure SQL", () => {
    const sql = readSource("azure", "azure-schema.sql");
  ...
  it("defines branch graph tables for Oracle", () => {
    const sql = readSource("oracle", "oracle-schema.sql");
