...
describe("shared SQL storage helpers", () => {
  it("provides provider-independent setting loaders through the base class", async () => {
    ...
    await expect(storage.loadPersonas()).resolves.toEqual({
    ...
    storage.pluginCustomStorageCache = {
      pluginCustomStorage: { k: "v" },
      hash: "def",
    };
  ...
  it("preserves provider-specific identifier and cold-storage limits", () => {
    const strict = createSqlStorageHelpers({
    ...
    expect(() => strict.normalizeColdStorageKey(shortKey)).toThrow(
      TestPayloadError,
    );
  ...
  it("validates message objects consistently for every provider", () => {
    ...
    expect(
      helpers.validateColdStorageValue({ message: [{ role: "user" }] }),
    ).toEqual({
      message: [{ role: "user" }],
    });
    expect(() => helpers.validateColdStorageValue({ message: [null] })).toThrow(
      TestPayloadError,
    );
  });
        ...
        upserts: [
          {
            ...
            position: 0,
            data: {
              name: "Only summary fields escape list API",
              openAIKey: "secret",
            },
          },
        ],
  ...
  it("rebuilds the message, chat, and character graph with load-state flags", () => {
    ...
    const messageRelations = createMessageRelations({
      attributes: [
        { chat_id: "chat-1", message_id: "message-1", value: "attribute" },
      ],
      generations: [],
      promptInfos: [],
      promptToggles: [],
    ...
    rebuildDatabaseGraph({
      ...
      characterRelations: {
        tags: new Map([["character-1", [{ tag: "test" }]]]),
      },
      chatRelations: { bookmarks: new Map() },
      messageRelations,
      rebuildMessage: (row: object, relations: object) => ({
        ...row,
        relations,
      }),
      rebuildChat: (row: object, relations: object) => ({ ...row, relations }),
      rebuildCharacter: (row: object, relations: object) => ({
        ...row,
        relations,
      }),
    });
