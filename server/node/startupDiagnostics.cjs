...
function readStorageStartupSettings(env = process.env) {
    ...
    const defaultConnectTimeoutMs = Math.min(DEFAULT_STORAGE_CONNECT_TIMEOUT_MS, startupTimeoutMs);
    return {
        startupTimeoutMs,
...
function sanitizeSensitiveText(value) {
    ...
    return String(value)
        ...
        .replace(/\b(password|passwd|pwd|secret|token|access[_-]?key|api[_-]?key)=([^\s&,;]+)/gi, '$1=<redacted>');
}
...
function describePostgresTarget(connectionString) {
    ...
    try {
        ...
        const database = decodeURIComponent(target.pathname.replace(/^\//, '')) || '(default database)';
...
function describeOracleTarget(tnsAlias, walletPath = '') {
    const alias = typeof tnsAlias === 'string' && /^[A-Za-z0-9_.-]+$/.test(tnsAlias)
        ? tnsAlias
...
function describeStorageTarget(vendor, storage) {
    ...
    if (vendor === 'postgres') return describePostgresTarget(storage.connectionString);
    if (vendor === 'oracle') {
...
function startupErrorHint(error) {
    ...
    if (['ETIMEDOUT', 'ESOCKET'].includes(code) || message.includes('connection timeout') || message.includes('connection timed out')) {
...
async function runStartupStage(options, task) {
    ...
    logger.log(`[${scope}] ${operation} started${safeDetail ? ` (${safeDetail})` : ''}.`);

    let heartbeat = null;
    if (heartbeatMs > 0) {
        heartbeat = setInterval(() => {
            logger.warn(`[${scope}] ${operation} is still running (${formatDuration(Date.now() - startedAt)} elapsed).`);
        }, heartbeatMs);
    }

    let timeout = null;
    const operationPromise = Promise.resolve().then(task);
    const guardedPromise = timeoutMs > 0
        ? Promise.race([
            operationPromise,
            new Promise((_, reject) => {
                timeout = setTimeout(() => reject(new StartupTimeoutError(scope, operation, timeoutMs)), timeoutMs);
            }),
        ])
        : operationPromise;
    ...
    try {
        ...
        logger.log(`[${scope}] ${operation} completed in ${formatDuration(Date.now() - startedAt)}.`);
        return result;
    } catch (error) {
        ...
        const message = sanitizeSensitiveText(error?.message || error || 'Unknown error');
        logger.error(`[${scope}] ${operation} failed after ${formatDuration(Date.now() - startedAt)}: ${message}`);
