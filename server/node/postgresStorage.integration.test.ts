...
describePostgres("PostgreSQL structured storage integration", () => {
  ...
  it("maintains an S3 asset catalog scoped to its storage source", async () => {
    ...
    const otherSource = JSON.stringify({
      type: "s3",
    ...
    await storage.replaceAssetCatalog(
      "assets/",
      [
  ...
  it("imports, reconstructs, and incrementally updates normalized data", async () => {
    const first = await storage.sync({
      ...
      root: {
        upserts: [
          ...
          {
            key: "translatorPresets",
            value: [{ name: "Korean", prompt: "Translate", maxResponse: 2048 }],
          },
    ...
    expect(loaded.database?.modules?.[0]?.mcp).toEqual({
      url: "https://mcp.test",
    });
    ...
    expect(loaded.database?.characters[0].alternateGreetings).toEqual([
      "Alt 1",
      "Alt 2",
    ]);
    expect(loaded.database?.characters[0].emotionImages).toEqual([
      ["happy", "asset://happy"],
    ]);
    ...
    expect(loaded.database?.characters[0].chats[0].message[0].data).toBe(
      "hello",
    );
    ...
    expect(updated.database?.characters[0].alternateGreetings).toEqual([
      "Alt 1",
    ]);
    expect(updated.database?.characters[0].chats[0].name).toBe("Renamed Chat");
    expect(updated.database?.characters[0].chats[0].message[0].data).toBe(
      "hello again",
    );
  });
  ...
  it("persists complex character assets, generation stats, and relational lore into dedicated tables", async () => {
    await storage.sync({
      ...
      chatManifests: [
        { characterId: "character-assets", ids: ["chat-assets"] },
      ],
      chats: [
        {
          id: "chat-assets",
  ...
  it("stores cold data in relational chat and message tables and prunes it with one set operation", async () => {
    ...
    expect((await storage.loadColdStorage(retainedKey))?.data).toEqual(
      coldCharacter,
    );
    expect(
      await storage.pool.query(
    ...
    expect(await storage.pruneColdStorage([retainedKey])).toEqual({
      deleted: 1,
    });
    expect((await storage.listColdStorage()).map((item) => item.key)).toEqual([
      retainedKey,
    ]);
  });
  ...
  it("imports each legacy compressed file once without resurrecting pruned data", async () => {
    ...
    try {
      await writeFile(
        ...
        deflateSync(
          JSON.stringify({ message: [{ role: "char", data: "legacy" }] }),
        ),
      );
  ...
  it("exports all PostgreSQL cold storage entries to legacy compressed files losslessly", async () => {
        ...
        chats: [
          {
            id: "chat-export",
            message: [
              { chatId: "msg-export", role: "user", data: "Exported text" },
            ],
          },
        ],
      },
    ...
    try {
      ...
      const filename = Buffer.from(`coldstorage/${key}`, "utf8").toString(
        "hex",
      );
  ...
  it("records fine-grained row-level audit logs for all mutations and restores prior states", async () => {
    await storage.sync({
      ...
      root: {
        upserts: [
          ...
          {
            key: "translatorPresets",
            value: [{ name: "Old", prompt: "old", maxResponse: 100 }],
          },
          {
            key: "loadouts",
            value: [
      ...
      characterIds: ["character-old"],
      characters: [
        { id: "character-old", position: 0, data: { name: "Old Character" } },
      ],
      chats: [],
      chatManifests: [],
      messages: [],
    ...
    await storage.sync({
      ...
      root: {
        upserts: [
          ...
          {
            key: "translatorPresets",
            value: [{ name: "New", prompt: "new", maxResponse: 200 }],
          },
          {
            key: "loadouts",
            value: [
    ...
    expect(
      ...
      rows: [
        { no_document_columns: true, has_assets: true, has_generation: true },
      ],
    });
    ...
    expect(
      (await storage.exportDatabaseSnapshot()).database?.translatorPresets,
    ).toEqual([{ name: "Old", prompt: "old", maxResponse: 100 }]);
    expect(
      await storage.pool.query(
        `SELECT
  ...
  it("restores relational message children and cold archives across multiple later revisions", async () => {
    ...
    await storage.sync({
      ...
      chatManifests: [
        { characterId: "character-restore", ids: ["chat-restore"] },
      ],
      chats: [
        {
          id: "chat-restore",
      ...
      messages: [
        {
          ...
          data: {
            role: "user",
            data: "after",
            generationInfo: { model: "new-model" },
          },
        },
      ],
    });
    await storage.upsertColdStorage(coldKey, {
    ...
    expect(
      (await storage.exportDatabaseSnapshot()).database?.characters[0].chats[0]
        .message[0],
    ).toMatchObject({
      data: "after",
      generationInfo: { model: "new-model" },
    ...
    expect(
      (await storage.exportDatabaseSnapshot()).database?.characters[0],
    ).toMatchObject({
      vits: null,
      extensionLabel: "old",
    });
  ...
  it("searches active and cold messages with full-text matching", async () => {
    await storage.sync({
      ...
      chatManifests: [
        { characterId: "character-search", ids: ["chat-search"] },
      ],
      chats: [
        {
          id: "chat-search",
          characterId: "character-search",
          position: 0,
          data: { name: "Search chat" },
        },
      ],
    ...
    await storage.upsertColdStorage("7be57c70-e119-4b7c-874f-68fd4d0d93c6", {
      message: [
        { role: "char", data: "a completely different archived sentence" },
      ],
    });
    ...
    expect(
      active.some(
        (r) => r.storageState === "active" && r.characterName === "Searchable",
      ),
    ).toBe(true);
  ...
  it("aggregates token usage across active and cold messages", async () => {
    await storage.sync({
      ...
      characters: [
        { id: "character-tokens", position: 0, data: { name: "Tokens" } },
      ],
      chatManifests: [
        { characterId: "character-tokens", ids: ["chat-tokens"] },
      ],
      chats: [
        {
          id: "chat-tokens",
          characterId: "character-tokens",
          position: 0,
          data: { name: "Token chat" },
        },
      ],
      ...
      message: [
        {
          role: "char",
          data: "world",
          generationInfo: { model: "model-a", inputTokens: 5, outputTokens: 7 },
        },
      ],
    });
