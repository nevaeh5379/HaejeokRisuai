...
describe("server startup diagnostics", () => {
  ...
  it("does not print full Oracle descriptors that may contain sensitive fields", () => {
    expect(describeOracleTarget("RISU_PROD", "/wallet")).toBe(
      "RISU_PROD (wallet configured)",
    );
    expect(
      describeOracleTarget("(DESCRIPTION=(ADDRESS=(HOST=db.internal)))"),
    ).toBe("configured TNS descriptor");
  });
  ...
  it("fails a stuck stage with a named timeout and actionable hint", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining("Apply schema failed"),
    );
    expect(startupErrorHint({ code: "RISUAI_STARTUP_TIMEOUT" })).toContain(
      "last progress line",
    );
    expect(
      startupErrorHint({
