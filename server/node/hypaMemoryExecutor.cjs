...
function normalizeMessage(message, index) {
  ...
  if (!['system', 'user', 'assistant', 'function'].includes(message.role) || typeof message.content !== 'string') {
    throw new TypeError(`chats[${index}] has an invalid role or content`);
  }
  return { ...message, memo: typeof message.memo === 'string' ? message.memo : '' };
}

function normalizeStartRequest(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new TypeError('Request body must be an object');
  if (!['legacy', 'v2', 'v3'].includes(input.mode)) throw new TypeError('mode must be legacy, v2, or v3');
  if (!Array.isArray(input.chats) || input.chats.length > 4096) throw new TypeError('chats must be an array of at most 4096 messages');
  const chats = input.chats.map(normalizeMessage);
  for (const key of ['currentTokens', 'maxContextTokens']) {
    if (!Number.isFinite(input[key]) || input[key] < 0 || input[key] > 10_000_000) throw new TypeError(`${key} is invalid`);
  }
  if (!input.config || typeof input.config !== 'object' || Array.isArray(input.config)) throw new TypeError('config must be an object');
...
function appendEmbeddingsPath(url) {
  return String(url).endsWith('/embeddings') ? String(url) : `${String(url).replace(/\/+$/, '')}/embeddings`;
}
...
async function fetchJson(url, options) {
  ...
  if (!response.ok) throw new Error(`Embedding HTTP ${response.status}: ${typeof data === 'string' ? data : JSON.stringify(data)}`);
  return data;
}
...
async function embedRegular(texts, config) {
  ...
  if (model === 'voyageContext3') {
    const groups = await embedVoyageGroups(texts.map((text) => [text]), config, 'document');
  ...
  if (model === 'custom') {
    if (!config.customEmbedding?.url) throw new Error('Custom model requires a Custom Server URL');
    ...
    apiModel = ({ ada: 'text-embedding-ada-002', openai3small: 'text-embedding-3-small', openai3large: 'text-embedding-3-large' })[model];
  }
  ...
  for (let offset = 0; offset < texts.length; offset += 50) {
    const data = await fetchJson(url, {
      method: 'POST',
      headers,
      body: JSON.stringify({ input: texts.slice(offset, offset + 50), ...(apiModel ? { model: apiModel } : {}) }),
    });
...
async function embedVoyageGroups(groups, config, inputType = 'document') {
  ...
  for (const group of groups) {
    if (batch.length > 0 && (batch.length >= 1000 || chunkCount + group.length > 16000)) {
      batches.push(batch);
      batch = [];
      chunkCount = 0;
    }
  ...
  for (const inputs of batches) {
    const data = await fetchJson('https://api.voyageai.com/v1/contextualizedembeddings', {
      method: 'POST',
      headers: { authorization: `Bearer ${key}`, 'content-type': 'application/json' },
      body: JSON.stringify({ model: 'voyage-context-3', inputs, input_type: inputType }),
    });
    if (!Array.isArray(data?.data)) throw new Error('Voyage embedding response has no data array');
    for (const group of data.data) output.push(group.data.map((item) => item.embedding));
  }
  return output;
}

function descriptorRevision(descriptors) {
  return hash(descriptors.map((item) => `${item.id}:${item.signature}`).join('\n'));
}

async function prepareIndex(scope, indexId, documents, config, contextualGroups = null) {
  ...
  let status = cached.ready ? cached : syncVectorIndex(scopedId, descriptors, revision);
  if (status.missingIds.length > 0) {
    const missing = new Set(status.missingIds.map(Number));
    const entries = [];
    if (normalizeHypaModel(config.hypaModel) === 'voyageContext3' && contextualGroups) {
      for (const indexes of contextualGroups) {
        if (!indexes.some((index) => missing.has(index))) continue;
        const vectors = (await embedVoyageGroups([indexes.map((index) => documents[index])], config, 'document'))[0];
        indexes.forEach((index, pos) => {
          if (missing.has(index)) entries.push({ id: String(index), signature: descriptors[index].signature, embedding: vectors[pos] });
        });
      }
    } else {
      const indexes = [...missing].sort((a, b) => a - b);
      const vectors = await embedRegular(indexes.map((index) => documents[index]), config);
      indexes.forEach((index, pos) => entries.push({ id: String(index), signature: descriptors[index].signature, embedding: vectors[pos] }));
    }
    for (let offset = 0; offset < entries.length; offset += 64) upsertVectorIndex(scopedId, entries.slice(offset, offset + 64));
  }
  return scopedId;
}
...
function parseChatMLRaw(data) {
  ...
  const messages = trimmed.split(starter).filter(Boolean).map((raw) => {
    let value = raw;
    let role = 'user';
    for (const candidate of ['user', 'system', 'assistant']) {
      ...
      if (value.startsWith(candidate + ' ') || value.startsWith(candidate + '\n')) {
        role = candidate;
    ...
    const thoughts = [];
    value = value.replace(/<Thoughts>(.+)<\/Thoughts>/gms, (_, thought) => { thoughts.push(thought); return ''; });
...
function validateCounts(value, expected) {
  if (!Array.isArray(value) || value.length !== expected || value.some((count) => !Number.isFinite(count) || count < 0)) {
...
async function* summarizeTextWithClient(text, prompt, model, isV3 = false) {
  ...
  output = output.replace(/<Thoughts>[\s\S]*?<\/Thoughts>/g, '').replace(/<think>[\s\S]*?<\/think>/g, '').trim();
...
async function directV2Summary(text, config) {
  ...
  const model = config.supaModelType === 'curie' ? 'text-curie-001' : config.supaModelType === 'instruct35' ? 'gpt-3.5-turbo-instruct' : 'text-davinci-003';
  const data = await fetchJson('https://api.openai.com/v1/completions', {
    method: 'POST',
    headers: { 'content-type': 'application/json', authorization: `Bearer ${config.supaMemoryKey || ''}` },
    body: JSON.stringify({ model, prompt: promptbody, max_tokens: 600, temperature: 0 }),
  });
...
async function* summarizeV2(text, config) {
  if (config.supaModelType === 'distilbart') {
    ...
    if (!result || result.ok !== true || typeof result.text !== 'string') throw new Error(result?.error || 'DistilBART summarization failed');
    return result.text;
  }
...
async function* runLegacy(request, context) {
  ...
  const prefixTokens = (count) => chatTokenCounts.slice(0, Math.max(0, count)).reduce((sum, value) => sum + value, 0);
  ...
  if (typeof request.room.supaMemoryData === 'string' && request.room.supaMemoryData.length > 4) {
    ...
    if (id.startsWith('hypa:')) {
      try {
        ...
        return { currentTokens, chats, error: 'hypaMemory: hypaMemory is not valid JSON' };
      }
      if (!Array.isArray(hypaData)) return { currentTokens, chats, error: "hypaMemory: hypaMemory isn't Array" };
      let selected = -1;
      for (let index = 0; index < hypaData.length; index++) {
        const chatIndex = chats.findIndex((chat) => chat.memo === hypaData[index]?.id);
      ...
      if (selected === -1) return { currentTokens, chats, error: 'hypaMemory: chat ID not found' };
      supaMemory = String(hypaData[selected]?.supa || '');
      hypaChunks = Array.isArray(hypaData[selected]?.hypa) ? hypaData[selected].hypa.slice() : [];
    }
  ...
  let hypaResult = '';
  hypaChunks = hypaChunks.filter((value) => typeof value === 'string' && value.length > 1);
  if (hypaChunks.length > 0) {
    ...
    for (const value of hypaChunks) {
      ...
      seen.add(value);
      const normalized = config.removePunctuationHypa ? stripHypaPunctuation(value) : value;
      if (stripHypaPunctuation(supaMemory).includes(stripHypaPunctuation(normalized))) continue;
      retrievalTexts.push(normalized);
    }
    if (retrievalTexts.length > 0) {
      const filteredChat = chats.filter((chat) => chat.role !== 'system' && chat.role !== 'function');
      const query = legacyStringlizeChat(filteredChat.slice(0, 4), request.character.name || '');
      ...
      const selectedTexts = (ranked[0] || []).map(([index]) => retrievalTexts[index]).slice(0, 3);
      if (selectedTexts.length > 0) {
        hypaResult = `past events: ${selectedTexts}`;
        currentTokens += (yield* tokenize([{ role: 'assistant', content: hypaResult, memo: 'hypaMemory' }]))[0];
        currentTokens += 10;
      }
    }
  }
  ...
  if (currentTokens < maxContextTokens) {
    chats.unshift({ role: 'system', content: `${supaMemory}\n\n${hypaResult}`, memo: 'supaMemory' });
  ...
  while (currentTokens > maxContextTokens) {
    ...
    if (Number(config.maxSupaChunkSize || 0) < maxChunkSize) maxChunkSize = Number(config.maxSupaChunkSize || 0);
    ...
    while (true) {
      ...
      if (!cont) {
        ...
        spiceLen = 0;
        if (summarized) {
          if (maxChunkSize < 500) return { currentTokens, chats, error: 'Not Enough Tokens to summarize in SupaMemory' };
          maxChunkSize *= 0.7;
        } else {
          let result;
          try { result = yield* summarizeLegacy(supaMemory, config); }
          catch (error) { return { currentTokens, chats, error: `SupaMemory: HTTP: ${error}` }; }
      ...
      if (chunkSize + tokens > maxChunkSize) {
        if (stringlizedChat === '' && cont.role !== 'function' && cont.role !== 'system') {
          const speaker = cont.role === 'assistant'
            ? (request.character.type === 'character' ? request.character.name || '' : '')
            : config.userName || '';
    ...
    if (stringlizedChat !== '') {
      let result;
      try { result = yield* summarizeLegacy(stringlizedChat, config); }
      catch (error) { return { currentTokens, chats, error: `SupaMemory: HTTP: ${error}` }; }

      const resultTokens = yield* plainCount(`${result}\n\n`);
      hypaChunks.push(result.replace(/\n+/g, '\n'));
      let supaList = supaMemory.split('\n\n').filter((value) => value.length > 1);
      if (supaList.length >= 3) {
        const oldSupa = supaMemory;
        try { supaMemory = yield* summarizeLegacy(supaMemory, config); }
        catch (error) { return { currentTokens, chats, error: `SupaMemory: HTTP: ${error}` }; }
  ...
  if (hypaResult !== '') chats.unshift({ role: 'system', content: hypaResult, memo: 'hypaMemory' });
...
function normalizeV2Data(raw, chats) {
  ...
  if (Array.isArray(raw.mainChunks) && raw.mainChunks.every((chunk) => typeof chunk?.targetId === 'string')) {
    ...
    for (const old of oldMainChunks) {
      ...
      const start = previousTarget ? chats.findIndex((chat) => chat.memo === previousTarget) : 0;
      if (end < 0 || start < 0) continue;
      const lo = previousTarget ? Math.min(start, end) : 0;
      const hi = Math.max(start, end);
      const id = data.lastMainChunkID++;
      data.mainChunks.push({ id, text: old.text, chatMemos: chats.slice(lo, hi + 1).map((chat) => chat.memo), lastChatMemo: old.targetId });
      for (const chunk of oldChunks.filter((chunk) => chunk.targetId === old.targetId)) data.chunks.push({ mainChunkID: id, text: chunk.text });
  ...
  return {
    ...
    chunks: Array.isArray(raw.chunks) ? raw.chunks.map((chunk) => ({ ...chunk })) : [],
    mainChunks: Array.isArray(raw.mainChunks) ? raw.mainChunks.map((chunk) => ({ ...chunk, chatMemos: Array.isArray(chunk.chatMemos) ? chunk.chatMemos.slice() : [] })) : [],
  };
}
...
async function* runV2(request, context) {
  ...
  data.mainChunks = data.mainChunks.filter((chunk) => isSubset(chunk.chatMemos, memoSet));
  ...
  if (data.mainChunks.length > 0) {
    const lastIndex = chats.findIndex((chat) => chat.memo === data.mainChunks.at(-1).lastChatMemo);
    if (lastIndex >= 0) {
      idx = lastIndex + 1;
      currentTokens -= chatTokenCounts.slice(0, lastIndex + 1).reduce((a, b) => a + b, 0);
    }
  }
  let failures = 0;
  while (currentTokens > maxContextTokens) {
    ...
    while (batchTokens < chunkSize && idx < chats.length - 4) {
      ...
      if (idx === 0 || !chat.content.trim()) { idx++; continue; }
      if (batchTokens + tokens > chunkSize) break;
      batch.push(chat); batchTokens += tokens; idx++;
    }
    if (batch.length === 0) {
      const message = idx >= chats.length - 4
    ...
    try {
      const summary = yield* summarizeV2(batch.map((chat) => `${chat.role}: ${chat.content}`).join('\n'), config);
      failures = 0;
      const summaryTokens = (yield* tokenize([{ role: 'system', content: summary }]))[0];
      void summaryTokens;
      data.lastMainChunkID++;
      const id = data.lastMainChunkID;
      data.mainChunks.push({ id, text: summary, chatMemos: batch.map((chat) => chat.memo), lastChatMemo: batch.at(-1).memo });
      for (const text of summary.split('\n\n').map((value) => value.trim()).filter(Boolean)) data.chunks.push({ mainChunkID: id, text });
      currentTokens -= batchTokens;
    } catch (error) {
      if (++failures >= 3) return { currentTokens, chats, error: '[HypaV2] Summarization failed multiple times. Aborting to prevent infinite loop.' };
    }
  }
  const mainCandidates = data.mainChunks.map((chunk) => ({ role: 'system', content: chunk.text }));
  ...
  const documents = data.chunks.filter((chunk) => chunk.text.trim()).map((chunk) => prefix + chunk.text.trim());
  const recentQueries = [];
  for (let i = 0; i < 3; i++) {
    const chat = chats[chats.length - i - 1];
    if (chat) recentQueries.push(`search_query: ${chat.content}`);
  }
  const rankedLists = await rankDocuments(context.scope, `hypav2:${request.character.id || ''}:${request.room.id || ''}`, documents, recentQueries, config, { metric: 'dot' });
  const scoreMap = new Map();
  rankedLists.forEach((rows, listIndex) => rows.forEach(([docIndex, score]) => scoreMap.set(docIndex, (scoreMap.get(docIndex) || 0) + score / (listIndex + 1))));
  const rankedIndexes = [...scoreMap.entries()].sort((a, b) => b[1] - a[1]).map(([index]) => index);
  const candidateTexts = rankedIndexes.map((index) => documents[index].substring(prefix.length));
  const candidateCounts = yield* tokenize(candidateTexts.map((content) => ({ role: 'system', content })));
  let details = '';
  let detailTokens = 0;
  for (let i = 0; i < candidateTexts.length; i++) {
    if (candidateCounts[i] > allocatedTokens - mainPromptTokens - detailTokens) break;
  ...
  currentTokens += (yield* tokenize([{ role: 'system', content: fullResult }]))[0];
  const resultChats = [{ role: 'system', content: fullResult, memo: 'supaMemory' }, ...chats.slice(idx)];
  for (const chat of lastTwoChats) if (!resultChats.some((item) => item.memo === chat.memo)) resultChats.push(chat);
  currentTokens -= allocatedTokens;
...
function normalizeV3Data(raw) {
  return {
    ...
    summaries: Array.isArray(raw?.summaries) ? raw.summaries.map((summary) => ({ ...summary, chatMemos: Array.isArray(summary.chatMemos) ? summary.chatMemos.slice() : [] })) : [],
  };
}
...
function splitBySeparator(text, separator) {
  try {
    ...
    return String(text).split(match ? new RegExp(match[1], match[2]) : new RegExp(separator));
  } catch { return String(text).split('\n\n'); }
}

function weightedRank(lists, weightFn) {
  const scores = new Map();
  lists.forEach((list, listIndex) => list.forEach(([item, score]) => scores.set(item, (scores.get(item) || 0) + score * weightFn(listIndex, lists.length))));
...
function addWithinBudget(candidates, tokenMap, budget, output) {
  ...
  for (const item of candidates) {
    ...
    output.push(item); used += tokens;
  }
  return used;
}

async function rankV3Summaries(request, context, data, selected, settings, chats, experimental) {
  const unused = data.summaries.map((summary, index) => ({ summary, index })).filter(({ summary }) => !selected.includes(summary));
  const docs = [];
  const contextualGroups = [];
  for (const { summary, index } of unused) {
    const group = [];
    for (const chunk of splitBySeparator(summary.text, settings.summaryChunkSeparator).map((value) => value.trim()).filter(Boolean)) {
  ...
  const groups = normalizeHypaModel(request.config.hypaModel) === 'voyageContext3' ? contextualGroups : null;
  if (experimental) {
    const recent = chats.slice(-settings.queryChatCount).filter((chat) => chat.content.trim());
    ...
    const lists = await rankDocuments(context.scope, `hypav3-exp:${request.character.id || ''}:${request.room.id || ''}`, docs.map((doc) => doc.text), queries.map((query) => query.content), request.config, { metric: 'cosine', contextualGroups: groups });
    const rankedChildren = weightedRank(lists, (index) => queries[index].weight);
    return childToParentRRF(rankedChildren, (docIndex) => docs[docIndex].summaryIndex).map((index) => data.summaries[index]);
  }
  const recent = chats.slice(-settings.queryChatCount).filter((chat) => chat.content.trim());
...
async function* runV3(request, context) {
  ...
  let currentTokens = request.currentTokens - Number(config.maxResponse || 0);
  const maxContextTokens = request.maxContextTokens;
  if (Number(settings.recentMemoryRatio || 0) + Number(settings.similarMemoryRatio || 0) > 1) {
    return { currentTokens, chats, error: '[HypaV3] The sum of Recent Memory Ratio and Similar Memory Ratio is greater than 1.' };
  }
  ...
  if (!settings.preserveOrphanedMemory) {
    ...
    data.summaries = data.summaries.filter((summary) => isSubset(summary.chatMemos, memos));
  }
  const chatTokenCounts = yield* tokenize(chats);
  let startIdx = 0;
  ...
  const emptyTokens = (yield* tokenize([{ role: 'system', content: emptyMemory }]))[0];
  const memoryTokens = Math.floor(maxContextTokens * Number(settings.memoryTokensRatio || 0));
  let availableMemoryTokens;
  let reserveKind;
  if (experimental) {
    const reserve = data.summaries.length > 0 || currentTokens > maxContextTokens;
    availableMemoryTokens = reserve ? memoryTokens - emptyTokens : 0;
    if (reserve) { currentTokens += memoryTokens; reserveKind = 'full'; }
    else reserveKind = 'none';
  } else {
    const reserveEmpty = data.summaries.length === 0 && currentTokens + emptyTokens <= maxContextTokens;
    availableMemoryTokens = reserveEmpty ? 0 : memoryTokens - emptyTokens;
    if (reserveEmpty) { currentTokens += emptyTokens; reserveKind = 'empty'; }
    else { currentTokens += memoryTokens; reserveKind = 'full'; }
  }
  const targetTokens = maxContextTokens * (1 - Number(settings.extraSummarizationRatio || 0));
  ...
  if (experimental) {
    ...
    while (summarizationMode) {
      ...
      if (chats.length - startIdx <= Number(settings.queryChatCount || 0)) {
        if (currentTokens <= maxContextTokens) break;
        return { currentTokens, chats, error: `[HypaV3] Cannot summarize further: input token count (${currentTokens}) exceeds max context size (${maxContextTokens}), but minimum ${settings.queryChatCount} messages required.`, memory: data };
      }
      const batch = [];
      let tokens = 0;
      let index = startIdx;
      while (batch.length < Number(settings.maxChatsPerSummary || 1) && index < chats.length - Number(settings.queryChatCount || 0)) {
        const chat = chats[index];
        tokens += chatTokenCounts[index];
        if (chat.name !== 'example_user' && chat.name !== 'example_assistant' && chat.memo !== 'NewChatExample' && chat.memo !== 'NewChat' && chat.content.trim() && !(settings.doNotSummarizeUserMessage && chat.role === 'user')) batch.push(chat);
        index++;
      }
      if (currentTokens <= maxContextTokens && currentTokens - tokens < targetTokens) break;
      if (batch.length) batches.push(batch);
      currentTokens -= tokens;
      startIdx = index;
    }
    for (const batch of batches) {
      try {
        ...
        data.summaries.push({ text, chatMemos: batch.map((chat) => chat.memo), isImportant: false, tags: [] });
      } catch (error) {
        return { currentTokens, chats, error: `[HypaV3] Summarization failed: ${error}`, memory: data };
      }
    }
  } else {
    while (summarizationMode) {
      ...
      const end = Math.min(startIdx + Number(settings.maxChatsPerSummary || 1), chats.length - Number(settings.queryChatCount || 0));
      const batch = [];
      let tokens = 0;
      for (let i = startIdx; i < end; i++) {
        const chat = chats[i]; tokens += chatTokenCounts[i];
        if (chat.name !== 'example_user' && chat.name !== 'example_assistant' && chat.memo !== 'NewChatExample' && chat.memo !== 'NewChat' && chat.content.trim() && !(settings.doNotSummarizeUserMessage && chat.role === 'user')) batch.push(chat);
      }
      if (currentTokens <= maxContextTokens && currentTokens - tokens < targetTokens) break;
      if (batch.length) {
  ...
  if (!data.summaries.length) {
    const resultChats = experimental ? chats.slice(startIdx) : [{ role: 'system', content: emptyMemory, memo: 'supaMemory' }, ...chats.slice(startIdx)];
    return { currentTokens, chats: resultChats, memory: data };
  }
  const summaryMessages = data.summaries.map((summary) => ({ role: 'system', content: `${summary.text}\n\n` }));
  const summaryCounts = yield* tokenize(summaryMessages);
  const tokenMap = new Map(data.summaries.map((summary, index) => [summary, summaryCounts[index] || 0]));
  const selected = [];
  const important = data.summaries.filter((summary) => summary.isImportant);
  let importantUsed = addWithinBudget(important, tokenMap, availableMemoryTokens, selected);
  ...
  if (recentRatio > 0) {
    const unused = data.summaries.filter((summary) => !selected.includes(summary)).reverse();
    usedRecent = addWithinBudget(unused, tokenMap, reservedRecent, recentSelected);
  ...
  if (similarRatio > 0) {
    let ranked;
    if (experimental) {
      ranked = await rankV3Summaries(request, context, data, selected, settings, chats, true);
    } else {
      const prep = await rankV3Summaries(request, context, data, selected, settings, chats, false);
      if (Array.isArray(prep)) ranked = prep;
        ...
        if (settings.enableSimilarityCorrection && prep.recent.length > 1) {
          try { queryTexts.push(yield* summarizeV3(prep.recent, settings)); }
          catch (error) { return { currentTokens, chats, error: `[HypaV3] Summarization failed: ${error}`, memory: data }; }
        }
        const lists = await rankDocuments(context.scope, `hypav3:${request.character.id || ''}:${request.room.id || ''}`, prep.docs.map((doc) => doc.text), queryTexts, config, { metric: 'dot', contextualGroups: prep.groups });
        const rankedChildren = weightedRank(lists, (index, total) => (index + 1) / ((total * (total + 1)) / 2));
        ranked = childToParentRRF(rankedChildren, (docIndex) => prep.docs[docIndex].summaryIndex).map((index) => data.summaries[index]);
      }
    }
    for (const summary of ranked || []) {
      const tokens = tokenMap.get(summary) || 0;
      if (tokens + usedSimilar > reservedSimilar) break;
      similarSelected.push(summary); usedSimilar += tokens;
    }
  ...
  if (randomRatio > 0) {
    reservedRandom += (reservedRecent - usedRecent) + (reservedSimilar - usedSimilar);
    const candidates = data.summaries.filter((summary) => !selected.includes(summary)).sort(() => Math.random() - 0.5);
    let used = 0;
    for (const summary of candidates) {
      const tokens = tokenMap.get(summary) || 0;
      if (tokens + used > reservedRandom) continue;
      selected.push(summary); randomSelected.push(summary); used += tokens;
    }
  }
  selected.sort((a, b) => data.summaries.indexOf(a) - data.summaries.indexOf(b));
  const memoryText = `<Past Events Summary>\n${selected.map((summary) => summary.text).join('\n\n')}\n</Past Events Summary>`;
  const realMemoryTokens = (yield* tokenize([{ role: 'system', content: memoryText }]))[0];
  ...
  if (currentTokens > maxContextTokens) throw new Error(`Unexpected error: input token count (${currentTokens}) exceeds max context size (${maxContextTokens})`);
  data.metrics = {
    lastImportantSummaries: important.filter((summary) => selected.includes(summary)).map((summary) => data.summaries.indexOf(summary)),
    lastRecentSummaries: recentSelected.map((summary) => data.summaries.indexOf(summary)),
    lastSimilarSummaries: similarSelected.map((summary) => data.summaries.indexOf(summary)),
    lastRandomSummaries: randomSelected.map((summary) => data.summaries.indexOf(summary)),
  };
  return { currentTokens, chats: [{ role: 'system', content: memoryText, memo: 'supaMemory' }, ...chats.slice(startIdx)], memory: data };
}

function createExecution(request, context) {
  if (request.mode === 'legacy') return runLegacy(request, context);
  return request.mode === 'v2' ? runV2(request, context) : runV3(request, context);
}

async function advanceSession(id, session, value, first = false) {
  session.lastAccess = Date.now();
  const step = first ? await session.generator.next() : await session.generator.next(value);
  if (step.done) {
    sessions.delete(id);
    return { status: 'done', result: step.value };
  }
  session.actionId = crypto.randomUUID();
  return { status: 'action', sessionId: id, action: { id: session.actionId, ...step.value } };
}
...
function createHypaMemoryExecutor() {
  async function start(rawRequest, context) {
    ...
    const session = { scope: context.scope, generator: createExecution(request, context), lastAccess: Date.now(), actionId: null };
    sessions.set(id, session);
    try { return await advanceSession(id, session, undefined, true); }
  ...
  async function resume(sessionId, actionId, value, context) {
    ...
    if (!session || session.scope !== context.scope) {
      const error = new Error('Hypa memory session not found'); error.code = 'hypa_session_missing'; throw error;
    }
    if (!actionId || actionId !== session.actionId) throw new TypeError('Hypa memory action id does not match');
    session.actionId = null;
    try { return await advanceSession(sessionId, session, value, false); }
  ...
  function registerRoutes(app, { auth, limiter, getScope }) {
    ...
    app.post('/api/hypa-memory/start', ...guards, async (req, res, next) => {
      ...
      try { res.send(await start(req.body, { scope: await getScope(req) })); }
      catch (error) {
        if (error instanceof TypeError || error instanceof RangeError) return res.status(400).send({ error: error.message });
        next(error);
      }
    });
    app.post('/api/hypa-memory/:sessionId/continue', ...guards, async (req, res, next) => {
      if (auth && !await auth(req, res)) return;
      try { res.send(await resume(req.params.sessionId, req.body?.actionId, req.body?.value, { scope: await getScope(req) })); }
      catch (error) {
        if (error?.code === 'hypa_session_missing') return res.status(404).send({ error: error.message, code: error.code });
        if (error instanceof TypeError || error instanceof RangeError) return res.status(400).send({ error: error.message });
        next(error);
      }
    });
    app.delete('/api/hypa-memory/:sessionId', ...guards, async (req, res, next) => {
      if (auth && !await auth(req, res)) return;
      try { cancel(req.params.sessionId, { scope: await getScope(req) }); res.status(204).end(); }
