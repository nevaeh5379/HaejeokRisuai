...
describe("AssetStorage utilities", () => {
  ...
  it("returns appropriate content type for various extensions", () => {
    ...
    expect(getContentType("database/database.bin")).toBe(
...
describe("LocalFsStorage", () => {
  ...
  it("rejects relative and absolute paths outside the storage directory", async () => {
    ...
    try {
      for (const input of [
        ...
        expect(await localStorage.openReadStream(input)).toEqual({
          exists: false,
        });
      }
      expect(createReadStream).not.toHaveBeenCalled();
  ...
  it("filters listed keys by prefix", async () => {
    ...
    await storage.write(
      keyToHex("database/database.bin"),
  ...
  it("generates, caches, and deletes thumbnails for image assets", async () => {
    ...
    const thumbResult1 = await storage.readThumbnail(hex, {
      width: 128,
      height: 128,
    });
    ...
    const thumbMetadata = await sharp(
      thumbResult1.buffer || fs.readFileSync(thumbResult1.filePath),
    ).metadata();
    ...
    const thumbResult2 = await storage.readThumbnail(hex, {
      width: 128,
      height: 128,
    });
...
describe("AssetStorageManager", () => {
  ...
  it('persists enabled: false on disk and switches to Local FS when setConfig({ storageType: "fs" }) is called', async () => {
    ...
    const savedS3 = JSON.parse(
      fs.readFileSync(path.join(tmpDir, "__s3_config.json"), "utf8"),
    );
    const savedAzure = JSON.parse(
      fs.readFileSync(path.join(tmpDir, "__azure_asset_config.json"), "utf8"),
    );
  ...
  return {
    send: async (command: any) => {
      ...
      if (cmdName.includes("ListObjectsV2")) {
        const keys = Array.from(s3Store.keys()).filter(
          (k) => !k.startsWith("thumbnails/"),
        );
        return {
          Contents: keys.map((k) => ({ Key: k, Size: s3Store.get(k)!.length })),
        };
      }
      return {};
    },
...
describe("S3AssetStorage readThumbnail", () => {
  ...
  it("fetches original, creates thumbnail, saves to S3, and returns buffer", async () => {
        ...
        if (cmdName === "GetObjectCommand" || cmdName.includes("GetObject")) {
          if (s3Store.has(cmdKey)) {
            ...
            return {
              ...
              ContentType: cmdKey.endsWith(".webp")
                ? "image/webp"
                : "image/png",
              ContentLength: buf.length,
            };
    ...
    const res1 = await s3Storage.readThumbnail(hex, {
      width: 128,
      height: 128,
    });
    ...
    const res2 = await s3Storage.readThumbnail(hex, {
      width: 128,
      height: 128,
    });
...
describe("createWriteStream streaming support", () => {
  ...
  it("S3AssetStorage.createWriteStream streams data into S3 Upload pipeline", async () => {
    ...
    s3Storage.client.send = (async (command: any) => {
      ...
      if (
        cmdName === "CreateMultipartUploadCommand" ||
        cmdName.includes("CreateMultipartUpload")
      ) {
      ...
      if (
        cmdName === "CompleteMultipartUploadCommand" ||
        cmdName.includes("CompleteMultipartUpload")
      ) {
        return {
    ...
    expect(s3Store.has(key)).toBe(true);
    expect(s3Store.get(key)!).toEqual(
      Buffer.from("stream-part-1-stream-part-2"),
    );
  });
  ...
  it("AzureSqlAssetStorage.createWriteStream preserves uploads with more than 100 chunks", async () => {
    ...
    expect(
      fs
        .readdirSync(tmpDir)
        .some((name) => name.startsWith(".__azuresql-upload-")),
    ).toBe(false);
  });
  ...
  it("S3AssetStorage eager thumbnail generation on write and local disk caching", async () => {
    ...
    s3Storage.client = {
      send: async (command: any) => {
        ...
        if (cmdName.includes("ListObjectsV2")) {
          const contents = Array.from(s3Store.keys()).map((k) => ({
            Key: k,
            Size: s3Store.get(k)!.length,
          }));
...
describe("S3AssetStorage parallel migrate / rollback", () => {
  ...
  it("migrateFromLocal uploads all local files in parallel and reports progress", async () => {
    ...
    const progressEvents: any[] = [];
    const result = await s3Storage.migrateFromLocal(localDir, (p) =>
      progressEvents.push(p),
    );
  ...
  it("migrateFromLocal skips files already present in S3 (batched exists check)", async () => {
    ...
    expect(result.errors).toEqual([]);
    expect(s3Store.get("assets/keep.bin")).toEqual(
      Buffer.from("already-there"),
    );
  ...
  it("rollbackToLocal downloads all S3 objects in parallel via stream pipeline", async () => {
    ...
    const progressEvents: any[] = [];
    const result = await s3Storage.rollbackToLocal(localDir, (p) =>
      progressEvents.push(p),
    );
  ...
  it("migrateFromLocal skips the exists check entirely when RISUAI_MIGRATE_SKIP_EXISTS_CHECK=1", async () => {
    ...
    try {
      ...
      expect(s3Store.get("assets/preexist.bin")).toEqual(
        Buffer.from("new-assets/preexist.bin"),
      );
      expect(s3Store.get("assets/fresh.bin")).toEqual(
        Buffer.from("new-assets/fresh.bin"),
      );
    } finally {
      if (prev === undefined)
        delete process.env.RISUAI_MIGRATE_SKIP_EXISTS_CHECK;
