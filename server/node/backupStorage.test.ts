...
const { PostgresStorage } = require("./postgresStorage.cjs") as {
  PostgresStorage: new (options: Record<string, any>) => Record<string, any>;
};
const { OracleStorage } = require("./oracleStorage.cjs") as {
  OracleStorage: new (options: Record<string, any>) => Record<string, any>;
};
const { AzureStorage } = require("./azureStorage.cjs") as {
...
describe("buildFullBackupPayload", () => {
  it("builds a replaceAll payload with the sync wire format", () => {
    ...
    const upsertKeys = payload.root.upserts
...
describe("backup config section normalization", () => {
  it("returns null for missing or invalid sections", () => {
    ...
    expect(
      normalizeBackupConfigSection({ vendor: "mysql", enabled: true }),
    ).toBeNull();
  });
  ...
  it("fills defaults and clamps the snapshot interval", () => {
    ...
    expect(backup).toMatchObject({
      ...
      snapshot: {
        enabled: true,
        intervalMinutes: MIN_BACKUP_SNAPSHOT_INTERVAL_MINUTES,
      },
    });
    expect(backup.params.connectionString).toBe("postgresql://u:p@h:5432/db");

    const defaults = normalizeBackupConfigSection({
      vendor: "oracle",
      enabled: false,
    });
    expect(defaults?.snapshot.intervalMinutes).toBe(
      DEFAULT_BACKUP_SNAPSHOT_INTERVAL_MINUTES,
    );
...
describe("stored config backup roundtrip", () => {
  it("persists and removes the backup section alongside primary config", () => {
    ...
    expect(stored.backup?.params.connectionString).toBe(
...
describe("isSecurePostgresConfigRequest logic", () => {
  function testIsSecureRequest(req: {
    ...
    if (
      typeof frontEndHttps === "string" &&
      frontEndHttps.toLowerCase() === "on"
    )
      return true;
  ...
  it("allows native secure connections and localhost", () => {
    ...
    expect(testIsSecureRequest({ socket: { remoteAddress: "::1" } })).toBe(
      true,
    );
    expect(
      testIsSecureRequest({ socket: { remoteAddress: "::ffff:127.0.0.1" } }),
    ).toBe(true);
  });
