...
- **DB 조각(fragment)**: 최대 256개의 데이터베이스 레코드를 담는 `database.stream/*.risudat` 엔트리
...
- `records`는 1개 이상 256개 이하여야 한다.
- 조각 번호와 레코드는 생성 순서를 유지해야 한다.

작성기가 조각 하나에 실제로 넣는 레코드 수는 성능 설정으로 조절할 수 있다. 기본값은 128개이며, 파일 형식의 안전 상한인 256개를 넘겨서는 안 된다. 이 값은 레코드 개수 상한일 뿐 바이트 크기 상한이 아니므로, 작성기는 개별 레코드가 매우 클 수 있음을 고려해야 한다.
