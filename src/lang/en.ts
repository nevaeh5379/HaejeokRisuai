export const languageEnglish = {
    ...
    postgresStorageDescription: "Store the application database and cold storage in PostgreSQL on the Node server.",
    usePostgresStorage: "Use PostgreSQL storage",
    postgresConnectionString: "PostgreSQL connection string",
    postgresConnectionSecurity: "The connection string is sent only over HTTPS or localhost to the authenticated Node server and is never stored in the Risuai application database.",
    ...
    postgresEnvironmentManaged: "This connection is managed by DATABASE_URL on the Node server. Change the server environment and restart to edit it.",
    ...
    postgresApplyConfirm: "Apply the PostgreSQL storage configuration? Risuai will create a current rollback snapshot and reload.",
    postgresApplySuccess: "PostgreSQL storage configuration was applied. Reloading...",
    postgresHistory: "Change history",
    postgresHistoryDescription: "Each save and cold-storage change is recorded as an immutable revision. Restoring creates another revision and never erases history.",
    postgresHistoryEmpty: "No revisions have been recorded yet.",
    postgresHistoryChanges: "row changes",
    postgresRestore: "Restore",
    postgresRestoreConfirm: (revision:number) => `Restore PostgreSQL data to revision #${revision}? The current state will remain in history and can be restored again.`,
    ...
    messageSearchUnavailable: "Message search requires PostgreSQL storage on the Node server.",
    postgresTokenUsage: "Token Usage",
    postgresTokenUsageDescription: "Aggregated token usage across active and cold-storage messages, grouped by model.",
    ...
    postgresDbExplorerDescription: "Browse the PostgreSQL tables and rows stored on the Node server. Read-only; changes are not possible here.",
    ...
    postgresDbExplorerDisabled: "Enable PostgreSQL storage in the Advanced settings to browse the database.",
    ...
    sqlStorageDescription: "Store the application database and cold storage in an external SQL database (PostgreSQL, Oracle, or Azure SQL) on the Node server.",
    ...
    sqlVendorPostgresDesc: "Open-source relational database. Provide a connection string.",
    sqlVendorOracleDesc: "Oracle Database. Provide user, password, and TNS alias.",
    sqlVendorAzureDesc: "Microsoft Azure SQL Database. Provide server, database, and credentials.",
    ...
    sqlSetupSkipped: "SQL setup skipped. You can configure it later in Advanced Settings.",
    ...
    sqlBackupDescription: "Keep a separate SQL database as a backup of the main database. Changes are mirrored in real time and/or snapshotted on an interval.",
    ...
    sqlBackupMirroringDescription: "Replicate every save (including cold storage) to the backup database right after it succeeds on the main database.",
    sqlBackupSnapshot: "Periodic full snapshot",
    sqlBackupSnapshotDescription: "Copy the entire main database to the backup database at a fixed interval.",
    sqlBackupSnapshotInterval: "Snapshot interval (minutes)",
    sqlBackupApply: "Apply Backup",
    sqlBackupApplyConfirm: "Apply the backup database configuration? The full contents of the main database will be copied to the backup database first.",
    sqlBackupApplySuccess: "Backup database configuration applied.",
    sqlBackupRemove: "Remove Backup",
    sqlBackupRemoveConfirm: "Remove the backup database configuration? Data already stored in the backup database is kept, but it will no longer be updated.",
    sqlBackupRemoveSuccess: "Backup database removed.",
    sqlBackupResync: "Full Backup Now",
    sqlBackupResyncConfirm: "Copy the entire main database to the backup database now? This may take a while for large datasets.",
    ...
    migrateLocalToSqlPrompt: "A SQL database is connected but empty. Migrate your current local data to the SQL database?",
    migrateLocalToSqlConfirm: "Migrate local data to the SQL database now? This may take a moment for large datasets.",
    migrateLocalToSqlSuccess: "Local data was migrated to the SQL database successfully.",
    migrateLocalToSqlSkipped: "Continuing with local data. The SQL database remains empty.",
    ...
    s3StorageDescription: "Store media assets (images, audio, attachments) in S3-compatible Object Storage (RustFS, MinIO, AWS S3, Cloudflare R2).",
    useS3Storage: "Use S3-compatible Object Storage",
    s3Endpoint: "S3 Endpoint URL",
    s3EndpointHint: "e.g. http://127.0.0.1:9000 (RustFS/MinIO) or leave empty for AWS S3",
    ...
    s3Apply: "Apply",
    s3Applying: "Applying...",
    s3ApplyConfirm: "Apply the storage configuration? Risuai will reload the server settings.",
    ...
    s3EnvironmentManaged: "This configuration is managed by environment variables on the Node server. Change the server environment and restart to edit it.",
    s3StatsAndTools: "Storage Statistics & Tools",
    s3StatsAndToolsDescription: "Inspect current storage usage and migrate assets between local disk and S3.",
    ...
    s3MigrateConfirm: "Upload all local assets from the server to the S3 bucket? Existing objects in S3 will be skipped.",
    s3MustBeEnabledToMigrate: "S3 storage must be enabled before migrating assets.",
    s3MigratingTitle: "Migrating Assets to S3...",
    s3MigrateSuccess: (migrated:number, skipped:number, total:number) => `Migration complete: ${migrated} uploaded, ${skipped} skipped (out of ${total} total).`,
    s3RollbackToLocal: "Download S3 Assets to Local",
    s3RollingBack: "Downloading...",
    s3RollingBackTitle: "Downloading Assets from S3...",
    s3RollbackConfirm: "Download all objects from the S3 bucket to the local server disk? This ensures local copies exist if reverting.",
    s3RollbackSuccess: (downloaded:number, total:number) => `Download complete: ${downloaded} downloaded (out of ${total} total).`,
    ...
    s3GenerateThumbnailsConfirm: "Pre-generate low-resolution WebP thumbnails for all image assets in S3 that lack thumbnails? This will significantly reduce initial loading traffic.",
    s3GenerateThumbnailsSuccess: (created:number, skipped:number, total:number) => `Thumbnail generation complete: ${created} created, ${skipped} skipped (out of ${total} total).`,
    storageExplorer: "Storage Explorer",
    storageExplorerDescription: "Manage local file system, S3/RustFS object storage, and monitor asset storage usage per bot and module.",
    ...
    storageCleanOrphanConfirm: (count: number, size: string) => `Permanently delete ${count} unreferenced asset file(s) (${size})? This action cannot be undone.`,
    storageCleanOrphanSuccess: (count: number, size: string) => `Successfully deleted ${count} unused asset(s), freeing ${size}.`,
    ...
    storageResyncCatalogDescription: "Re-list the S3 bucket and rebuild the SQL asset catalog",
    storageResyncCatalogConfirm: "Re-list the entire S3 bucket and rebuild the SQL asset catalog? This is required if the bucket was modified outside RisuAI.",
    storageEmptyCatalogConfirm: "The SQL asset catalog is empty. List the S3 bucket now and save its objects to the catalog?",
    storageResyncingCatalog: "Resyncing...",
    storageResyncCatalogSuccess: (count: number) => `Asset catalog resync complete: ${count} object(s) recorded.`,
    ...
    storageShowingRange: (start: number, end: number, total: number) => `Showing ${start}–${end} of ${total}`,
    storageSelectCurrentPage: "Select Current Page",
    storageSelectAllMatching: (total: number) => `Select all ${total} matching files`,
    ...
    storageDeleteConfirm: (count: number) => `Permanently delete ${count} selected asset file(s)?`,
    storageDeleteSuccess: (count: number) => `${count} asset(s) deleted successfully.`,
    ...
    storagePurgeLocalFsConfirm: (size: string) => `Permanently delete all asset files from local server disk to free ${size}? Please ensure assets are migrated to S3 first.`,
    storagePurgeLocalFsSuccess: (count: number, size: string) => `Deleted ${count} local assets, freeing ${size} of disk space.`,
    ...
    storageMissingAssetsDetected: (count: number) => `${count} missing asset(s) not found in storage.`,
    storageMissingBotsDetected: (count: number) => `${count} bot(s) have missing assets in storage.`,
    storageMissingModulesDetected: (count: number) => `${count} module(s) have missing assets in storage.`,
    storageFilterMissing: "Missing Assets",
    storageSortMissingDesc: "Most Missing Assets",
    storageAssetNotFoundInStorage: "File does not exist in the current storage backend.",
    ...
    storageBackendSelectDescription: "Choose where media assets are stored. Selecting a backend activates it immediately after applying.",
    ...
    storageBackendS3Desc: "Store assets in S3-compatible object storage (RustFS, MinIO, AWS S3, R2).",
    storageBackendAzureSqlDesc: "Store assets as blobs in Azure SQL / MSSQL database.",
    ...
    dbExplorerDisabledDesc: "Complete database configuration to enable external SQL storage.",
    ...
    dbStatsBotAnalyticsDesc: "Monitor chat sessions, message counts, average response lengths, and recent activity per bot.",
    ...
    dbStatsModelTokenUsageDesc: "Token consumption and request counts per AI model.",
    ...
    dbHistoryTimelineDesc: "View database change history and restore to a previous snapshot.",
    ...
    dbHistoryAuditNoChanges: "No row-level audit entries recorded for this revision.",
    ...
    dbHistoryRestoreModalDesc: "Review impact analysis before restoring your database to this revision.",
    dbHistoryRestoreSafeNotice: "Safety Notice: Restoring does NOT delete history. Your current state will be safely recorded as a new revision before applying changes.",
    ...
    sqlBackupRestoreToMainConfirm: "Are you sure you want to restore the backup database into the main database? The current content of the main database will be completely replaced.",
    sqlBackupRestoreToMainSuccess: "Successfully restored the backup database into the main database. Reloading...",
