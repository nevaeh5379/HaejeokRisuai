export const languageKorean = {
    ...
    "help": {
        ...
        "lowSpecMode": "구형 기기에서 로딩 이후 발생하는 멈춤을 줄입니다. 채팅 런타임 준비가 끝난 뒤 앱을 표시하고, 제한된 크기의 캐릭터 썸네일을 디코딩하며, 더 적은 채팅 메시지를 렌더링하고, 비활성 메시지를 유휴 시간에 나누어 정리합니다. 설정을 변경한 뒤 Risuai를 다시 시작하세요.",
    ...
    "postgresStorageDescription": "Node 서버의 애플리케이션 데이터베이스와 Cold Storage를 PostgreSQL에 저장합니다.",
    "usePostgresStorage": "PostgreSQL 저장소 사용",
    "postgresConnectionString": "PostgreSQL 연결 주소",
    "postgresConnectionSecurity": "연결 주소는 HTTPS 또는 localhost에서만 인증된 Node 서버로 전송되며 Risuai 애플리케이션 데이터베이스에는 저장되지 않습니다.",
    ...
    "postgresEnvironmentManaged": "이 연결은 Node 서버의 DATABASE_URL 환경 변수로 관리됩니다. 변경하려면 서버 환경 변수를 수정하고 재시작하세요.",
    ...
    "postgresApplyConfirm": "PostgreSQL 저장소 설정을 적용할까요? 현재 데이터의 롤백 스냅샷을 만든 뒤 앱을 새로고침합니다.",
    "postgresApplySuccess": "PostgreSQL 저장소 설정을 적용했습니다. 새로고침합니다...",
    "postgresHistory": "변경 이력",
    "postgresHistoryDescription": "저장 및 Cold Storage 변경을 불변 리비전으로 기록합니다. 복원도 새 리비전으로 남으며 기존 이력을 지우지 않습니다.",
    "postgresHistoryEmpty": "아직 기록된 리비전이 없습니다.",
    "postgresHistoryChanges": "개 행 변경",
    "postgresRestore": "복원",
    "postgresRestoreConfirm": (revision:number) => `PostgreSQL 데이터를 리비전 #${revision} 상태로 복원할까요? 현재 상태도 이력에 남아 다시 복원할 수 있습니다.`,
    ...
    "messageSearchUnavailable": "메시지 검색은 Node 서버의 PostgreSQL 저장소가 필요합니다.",
    "postgresTokenUsage": "토큰 사용량",
    "postgresTokenUsageDescription": "활성 및 콜드 스토리지 메시지의 모델별 토큰 사용량 집계입니다.",
    ...
    "postgresDbExplorerDescription": "Node 서버에 저장된 PostgreSQL 테이블과 행을 둘러봅니다. 읽기 전용이며 여기서 데이터를 수정할 수 없습니다.",
    ...
    "postgresDbExplorerDisabled": "고급 설정에서 PostgreSQL 저장소를 활성화하면 데이터베이스를 둘러볼 수 있습니다.",
    ...
    "sqlStorageDescription": "애플리케이션 데이터베이스와 Cold Storage를 외부 SQL 데이터베이스(PostgreSQL, Oracle, Azure SQL)에 저장합니다.",
    ...
    "sqlVendorPostgresDesc": "오픈소스 관계형 데이터베이스. 연결 문자열을 입력하세요.",
    "sqlVendorOracleDesc": "Oracle Database. 사용자, 비밀번호, TNS 별칭을 입력하세요.",
    "sqlVendorAzureDesc": "Microsoft Azure SQL Database. 서버, 데이터베이스, 자격 증명을 입력하세요.",
    ...
    "sqlSetupSkipped": "SQL 설정을 건너뛰었습니다. 고급 설정에서 나중에 구성할 수 있습니다.",
    "sqlSelectVendorFirst": "먼저 데이터베이스 제공자를 선택하세요.",
    "sqlConfigIncomplete": "모든 필수 연결 필드를 입력해주세요.",
    "sqlBackupTitle": "백업 데이터베이스",
    "sqlBackupDescription": "메인 데이터베이스의 백업으로 별도 SQL 데이터베이스를 유지합니다. 변경 사항은 실시간으로 미러링되고, 설정된 간격으로 전체 스냅샷이 생성됩니다.",
    "sqlBackupNotConfigured": "백업 데이터베이스가 설정되어 있지 않습니다.",
    "sqlBackupChooseVendor": "백업 데이터베이스 선택",
    "sqlBackupMirroring": "실시간 미러링",
    "sqlBackupMirroringDescription": "메인 데이터베이스 저장 성공 시 모든 변경 사항(콜드 스토리지 포함)을 즉시 백업 데이터베이스에 복사합니다.",
    "sqlBackupSnapshot": "주기적 전체 스냅샷",
    "sqlBackupSnapshotDescription": "설정된 간격으로 메인 데이터베이스 전체를 백업 데이터베이스에 복사합니다.",
    "sqlBackupSnapshotInterval": "스냅샷 간격(분)",
    "sqlBackupApply": "백업 적용",
    "sqlBackupApplyConfirm": "백업 데이터베이스 설정을 적용할까요? 먼저 메인 데이터베이스 전체가 백업 데이터베이스에 복사됩니다.",
    "sqlBackupApplySuccess": "백업 데이터베이스 설정을 적용했습니다.",
    "sqlBackupRemove": "백업 해제",
    "sqlBackupRemoveConfirm": "백업 데이터베이스 설정을 해제할까요? 백업 데이터베이스에 이미 저장된 데이터는 유지되지만 더 이상 갱신되지 않습니다.",
    "sqlBackupRemoveSuccess": "백업 데이터베이스를 해제했습니다.",
    "sqlBackupResync": "지금 전체 백업",
    "sqlBackupResyncConfirm": "지금 메인 데이터베이스 전체를 백업 데이터베이스에 복사할까요? 데이터가 크면 시간이 걸릴 수 있습니다.",
    ...
    "migrateLocalToSqlPrompt": "SQL 데이터베이스가 연결되었지만 비어 있습니다. 현재 로컬 데이터를 SQL 데이터베이스로 마이그레이션할까요?",
    "migrateLocalToSqlConfirm": "지금 로컬 데이터를 SQL 데이터베이스로 마이그레이션할까요? 데이터가 크면 시간이 조금 걸릴 수 있습니다.",
    "migrateLocalToSqlSuccess": "로컬 데이터를 SQL 데이터베이스로 마이그레이션했습니다.",
    "migrateLocalToSqlSkipped": "로컬 데이터를 계속 사용합니다. SQL 데이터베이스는 빈 상태로 유지됩니다.",
    ...
    "s3StorageDescription": "미디어 에셋(이미지, 오디오, 첨부파일 등)을 S3 호환 객체 스토리지(RustFS, MinIO, AWS S3, Cloudflare R2)에 저장합니다.",
    "useS3Storage": "S3 호환 객체 스토리지 사용",
    "s3Endpoint": "S3 엔드포인트 URL",
    "s3EndpointHint": "예: http://127.0.0.1:9000 (RustFS/MinIO) 또는 AWS S3의 경우 비워둠",
    "s3Bucket": "버킷 이름",
    ...
    "s3Apply": "적용",
    "s3Applying": "적용 중...",
    "s3ApplyConfirm": "스토리지 설정을 적용할까요? 서버 설정을 갱신하고 새로고침합니다.",
    "s3ApplySuccess": "스토리지 설정을 적용했습니다. 새로고침합니다...",
    "s3StatusLocalFs": "로컬 파일시스템",
    "s3StatusLoading": "S3 스토리지 상태를 불러오는 중...",
    "s3EnvironmentManaged": "이 설정은 Node 서버의 환경 변수로 관리됩니다. 변경하려면 서버 환경 변수를 수정하고 재시작하세요.",
    "s3StatsAndTools": "스토리지 통계 및 도구",
    "s3StatsAndToolsDescription": "현재 스토리지 사용량을 확인하고 로컬 디스크와 S3 간에 에셋을 마이그레이션합니다.",
    ...
    "s3MigrateConfirm": "서버 로컬의 모든 에셋을 S3 버킷으로 업로드할까요? S3에 이미 존재하는 객체는 건너뜁니다.",
    "s3MustBeEnabledToMigrate": "에셋을 마이그레이션하려면 먼저 S3 스토리지를 활성화해야 합니다.",
    "s3MigratingTitle": "로컬 에셋 → S3 마이그레이션 중...",
    "s3MigrateSuccess": (migrated:number, skipped:number, total:number) => `마이그레이션 완료: ${migrated}개 업로드, ${skipped}개 건너뜀 (총 ${total}개).`,
    "s3RollbackToLocal": "S3 에셋을 로컬로 다운로드",
    "s3RollingBack": "다운로드 중...",
    "s3RollingBackTitle": "S3 → 로컬 에셋 다운로드 중...",
    "s3RollbackConfirm": "S3 버킷의 모든 객체를 서버 로컬 디스크로 다운로드할까요? 로컬 모드로 되돌릴 때 유용합니다.",
    "s3RollbackSuccess": (downloaded:number, total:number) => `다운로드 완료: ${downloaded}개 다운로드됨 (총 ${total}개).`,
    "s3GenerateThumbnails": "미생성 썸네일 일괄 생성",
    "s3GeneratingThumbnails": "썸네일 생성 중...",
    "s3GenerateThumbnailsTitle": "S3 봇 아이콘 썸네일 생성 중...",
    "s3GenerateThumbnailsConfirm": "S3 버킷의 이미지 에셋 중 썸네일이 없는 파일들에 대해 저해상도 WebP 썸네일을 일괄 생성할까요? 초기 로딩 트래픽을 크게 절감할 수 있습니다.",
    "s3GenerateThumbnailsSuccess": (created:number, skipped:number, total:number) => `썸네일 생성 완료: ${created}개 생성됨, ${skipped}개 건너뜀 (총 ${total}개).`,
    "storageExplorer": "스토리지 탐색기",
    "storageExplorerDescription": "로컬 파일 시스템 및 S3/RustFS 객체 스토리지를 관리하고 캐릭터·모듈별 에셋 용량을 모니터링합니다.",
    ...
    "storageCleanOrphanConfirm": (count: number, size: string) => `어떤 캐릭터나 모듈에서도 사용하지 않는 미사용 에셋 ${count}개(${size})를 영구 삭제할까요? 이 작업은 되돌릴 수 없습니다.`,
    "storageCleanOrphanSuccess": (count: number, size: string) => `미사용 에셋 ${count}개를 정리하여 ${size}의 저장 공간을 확보했습니다.`,
    ...
    "storageResyncCatalogDescription": "S3 버킷을 다시 목록화하여 SQL 에셋 카탈로그를 재구성합니다",
    "storageResyncCatalogConfirm": "S3 버킷 전체를 다시 목록화하여 SQL 에셋 카탈로그를 재구성할까요? RisuAI 외부에서 버킷이 수정된 경우에 필요합니다.",
    "storageEmptyCatalogConfirm": "SQL 에셋 카탈로그가 비어 있습니다. 지금 S3 버킷 목록을 조회하여 카탈로그에 저장할까요?",
    "storageResyncingCatalog": "재동기화 중...",
    "storageResyncCatalogSuccess": (count: number) => `에셋 카탈로그 재동기화 완료: ${count}개 객체를 기록했습니다.`,
    ...
    "storageShowingRange": (start: number, end: number, total: number) => `전체 ${total}개 중 ${start}–${end}번째 표시`,
    ...
    "storageDeleteConfirm": (count: number) => `선택한 ${count}개의 에셋 파일을 영구 삭제할까요?`,
    "storageDeleteSuccess": (count: number) => `${count}개의 에셋이 삭제되었습니다.`,
    ...
    "storagePurgeLocalFsConfirm": (size: string) => `로컬 서버 디스크의 모든 에셋 파일을 삭제하여 ${size}의 공간을 확보할까요? 반드시 S3로 마이그레이션이 완료되었는지 확인하세요.`,
    "storagePurgeLocalFsSuccess": (count: number, size: string) => `로컬 에셋 ${count}개를 정리하여 ${size}의 디스크 공간을 확보했습니다.`,
    ...
    "storageMissingAssetsDetected": (count: number) => `스토리지에 존재하지 않는 에셋 ${count}개가 감지되었습니다.`,
    "storageMissingBotsDetected": (count: number) => `스토리지에 에셋이 누락된 봇이 ${count}개 있습니다.`,
    "storageMissingModulesDetected": (count: number) => `스토리지에 에셋이 누락된 모듈이 ${count}개 있습니다.`,
    ...
    "storageBackendSelectDescription": "미디어 에셋을 저장할 위치를 선택하세요. 백엔드를 선택하고 적용하면 즉시 활성화됩니다.",
    ...
    "storageBackendS3Desc": "S3 호환 객체 스토리지(RustFS, MinIO, AWS S3, R2)에 에셋을 저장합니다.",
    "storageBackendAzureSqlDesc": "Azure SQL / MSSQL 데이터베이스에 에셋을 blob으로 저장합니다.",
    ...
    "dbExplorerDisabledBanner": "데이터베이스가 설정되지 않았거나 비활성화되어 있습니다.",
    "dbExplorerDisabledDesc": "데이터베이스 설정을 완료하여 원격 SQL 저장소 기능을 활성화하세요.",
    ...
    "dbStatsBotAnalyticsDesc": "봇별 채팅 세션 수, 총 메시지 수, 평균 응답 길이 및 최근 활동을 분석합니다.",
    ...
    "dbStatsModelTokenUsageDesc": "각 AI 모델별 요청 횟수와 입력/출력 토큰 소비량 통계입니다.",
    ...
    "dbHistoryTimelineDesc": "데이터베이스 변경 이력을 확인하고 특정 시점으로 되돌립니다.",
    ...
    "dbHistoryRestoreModalDesc": "데이터베이스를 이 시점으로 복원하기 전 영향 분석을 확인하세요.",
    "dbHistoryRestoreSafeNotice": "안전 안내: 복원은 기존 이력을 삭제하지 않습니다. 현재 상태 또한 새로운 복원 리비전으로 안전하게 기록됩니다.",
    ...
    "sqlBackupRestoreToMainConfirm": "정말로 백업 데이터베이스의 내용을 메인 데이터베이스에 덮어쓸까요? 메인 데이터베이스의 현재 내용이 백업 데이터베이스의 내용으로 완전히 교체됩니다.",
    "sqlBackupRestoreToMainSuccess": "백업 데이터베이스의 내용을 메인 데이터베이스에 성공적으로 복원했습니다. 페이지를 새로고침합니다...",
