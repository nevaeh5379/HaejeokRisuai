export const languageChinese = {
  ...
  errors: {
    ...
    networkFetchPlain:
      "这可能是一次数据抓取（Fetch）错误。请尝试在设置中关闭强制抓取选项。",
  ...
  help: {
    ...
    submodel:
      "辅助模型是一个用于分析情感图像、产生自动建议等的模型，推荐使用 GPT-3.5。",
    oaiapikey:
    ...
    globalNote:
      "一个对模型行为有强烈影响的备注（也称为 UJB），适用于所有角色。",
    ...
    loreorder:
      "插入顺序越高，对模型的影响力越大。在激活大量条目时，也更不容易被截断。",
    ...
    creatorQuotes:
      "说明将显示在初始消息之上，用于向用户提供角色说明。此内容不会进入提示词中。",
    systemPrompt: "此字段不为空时，将替换设置中的主提示词为此内容。",
    chatNote:
      "这是一个强烈影响模型行为的备注，嵌入于当前聊天中，也称为记忆或 UJB。",
    personality:
      "对角色性格的简要描述。\n\n**不建议使用此字段，请填写在角色描述中。**",
    scenario:
      "对角色情境的简要描述。\n\n**不建议使用此字段，请填写在角色描述中。**",
    ...
    loreSelective:
      "激活选择性模式后，需同时匹配关键词与次要关键詞，方可激活该条目。",
    ...
    inlayImages:
      "激活后，图片可嵌入聊天中，并且支持此功能的 AI 将能够看到它们。",
    metrica:
    ...
    chatLoadInitialPages:
      "打开聊天界面时初始渲染的最近聊天消息数量。数值越高，能立即显示更多历史，但打开长聊天时可能更卡。",
    chatLoadAdditionalPages:
      "每次向上滚动到顶部时额外渲染的旧聊天消息数量。数值越高，重复加载次数越少，但每次加载可能更重。",
    ...
    dynamicAssetsEditDisplay:
      "激活后，动态资源将同样应用于“修改显示”阶段，但这可能会影响性能。",
    nickname:
      "设置后，将在聊天中以此昵称取代角色名称，并显示于 {{char}} 和 <char>。",
    useRegexLorebook:
    ...
    openAIFlexProcessing:
      "OpenAI Flex 响应可能比普通响应更慢，但可以用更低的价格（Batch API 价格）获得响应。此开关仅适用于官方 OpenAI Chat Completions 请求。",
    ...
    betaMobileGUI:
      "激活后，将在小于 800px 的屏幕上使用测试版行动接口，需刷新页面。",
  ...
  setup: {
    ...
    openRouterProvider:
      "OpenRouter 提供许多模型，部分免费且未经内容过滤，但质量不如 OpenAI。",
    hordeProvider: "Horde 提供免费服务，但回应时间较长且质量较低。",
    setProviderLater:
      "还有其他提供者，你可以稍后在设置中配置。如想稍后设置，请选择此选项。",
    setupOpenAI:
    ...
    openAIDesc:
      "OpenAI GPT 是高质量的 AI 模型，但它付费且存在内容过滤，在非英语环境下表现较弱。",
    ...
    chooseChatTypeOption2Desc:
      "推荐非英语使用者。AI 内部使用英语处理，但输入输出为您的语言。",
    ...
    dropDatabaseBinTitle: "将 database.bin 文件拖放到此处",
    dropDatabaseBinDesc: "或点击选择 database.bin / .risum / .risup 备份文件",
    selectFileBtn: "选择文件",
    migrationPreviewTitle: "导入数据预览",
    migrationCharacters: "角色",
    migrationChats: "对话记录",
    migrationPresets: "预设",
    migrationModules: "模块",
    migrationUsername: "用户名",
    ...
    selectAiModel: "选择 AI 模型提供者",
    selectAiModelDesc: "请选择用于聊天的 AI 模型提供者。",
    inputApiKeyTitle: "API 密钥与设置",
    inputApiKeyPlaceholder: "在此粘贴你的 API 密钥...",
    apiKeyLink: "前往获取 API 密钥",
    apiKeyDesc: "密钥仅保存在本地设备中，绝不会外泄。（稍后也可在设置中配置）",
    nicknameDesc: "请设置在对话中称呼你的名称。",
    nicknameInputPlaceholder: "请输入你的昵称（例：冒险者、User）",
    nextStep: "下一步",
    prevStep: "上一步",
    finishSetup: "启动 Haejeok RisuAI",
    allSetMessage: "全部设置已完成！与爱丽丝一起开始体验 Haejeok RisuAI 吧。",
    ...
    modelOpenRouterDesc: "OpenRouter (多模型集成平台)",
    modelCustomDesc: "自定义 / 反向代理 / OpenAI 兼容端点",
    modelOllamaDesc: "Ollama / 本地 (离线私密运行)",
    modelHordeDesc: "AI Horde (免费社区分布式模型)",
    modelLaterDesc: "稍后在设置中自行配置",
    badgeBackupFile: "备份文件",
    badgeDetected: "已检测到",
    badgeRecommended: "推荐",
    badgeNewRecommended: "推荐",
    badgePopular: "热门",
    badgeFreeTier: "免费额度",
    badgeFlexible: "多种模型",
    badgeLocalOffline: "本地 / 离线",
    badgeFree: "免费",
    badgeCustom: "自定义",
    actionRestore: "导入",
    actionStart: "开始设置",
    stepIndicator: "步骤",
    chatTranslationMode: "聊天翻译模式",
  ...
  triggerDesc: {
    ...
    v2ImgGen: "图像生成",
    v2ImgGenDesc:
      "从 {{value}} 生成图像，负面提示词 {{negValue}} => {{outputVar}}",
    v2CheckSimilarity: "检查相似度",
    v2CheckSimilarityDesc:
      "检查 {{source}} 和 {{value}} 之间的相似度 => {{outputVar}}",
    v2RunLLM: "运行模型",
    ...
    v2RegexTest: "正则测试",
    v2RegexTestDesc:
      "测试 {{value}} 是否匹配正则 {{regex}} 和标志 {{flags}} => {{outputVar}}",
    ...
    v2GetCharAtDesc:
      "从 {{source}} 获取索引为 {{index}} 的字符 => {{outputVar}}",
    ...
    v2SetCharAtDesc:
      "将 {{source}} 中索引为 {{index}} 的字符设置为 {{value}} => {{outputVar}}",
    v2SplitString: "分割字符串",
    ...
    v2GetArrayVarDesc:
      "获取数组变量 {{var}} 中索引为 {{index}} 的元素 => {{outputVar}}",
    v2SetArrayVar: "设置数组变量元素",
    v2SetArrayVarDesc:
      "将数组变量 {{var}} 中索引为 {{index}} 的元素设置为 {{value}}",
    ...
    v2GetAlertSelectDesc:
      "获取弹窗选择 {{display}} 选项 {{value}} => {{outputVar}}",
    ...
    v2GetRequestStateRoleDesc:
      "获取索引为 {{index}} 的请求数据角色 => {{outputVar}}",
    v2SetRequestStateRole: "设置请求数据角色",
    v2SetRequestStateRoleDesc:
      "将索引为 {{index}} 的请求数据角色设置为 {{value}}",
    ...
    v2SetLorebookAlwaysActiveDesc:
      "将索引为 {{index}} 的世界书始终激活设置为 {{value}}",
    v2Calculate: "计算",
    v2CalculateDesc: "计算 {{expression}} => {{outputVar}}",
    v2ReplaceString: "替换字符串",
    v2ReplaceStringDesc:
      "使用正则 {{regex}} 将 {{source}} 替换为 {{replacement}} => {{outputVar}}",
  },
  triggerInputLabels: {
    value: "值",
    var: "变量",
  ...
  openFloatingManager: "在浮动窗口中打开",
  assetManager: "额外资源管理器",
  filterAll: "全部",
  filterImages: "图片",
  filterAudio: "音频",
  filterVideo: "视频",
  ...
  tagCopied: "标签已复制到剪贴板",
  batchDelete: "批量删除",
  batchExclude: "切换提示词排除",
  selectAll: "全选",
  deselectAll: "取消全选",
  gridSize: "网格大小",
  dropFilesToUpload: "拖放文件到此处以上传",
  assetDetails: "资源详情",
  renameAsset: "重命名资源",
  duplicateNameWarn: "已存在同名资源",
  noAssetsFound: "未找到资源",
  searchAssets: "搜索资源...",
  sortBy: "排序",
  sortNameAsc: "名称 (A-Z)",
  ...
  macroFormats: "宏格式",
  promptInclusion: "提示词包含",
  promptIncluded: "包含在提示词中",
  promptExcluded: "从提示词中排除",
  deleteAssetConfirm: "确定要删除选中的资源吗？",
  delete: "删除",
  addAsset: "添加资源",
  totalAssetsCount: "{{count}} 个资源",
  viewGrid: "网格视图",
  viewList: "列表视图",
  smallSize: "小",
  mediumSize: "中",
  ...
  updateRealmDesc:
    "你正试图将角色更新至 RisuRealm。此操作将使角色更新至 RisuRealm，且无法还原。",
  antiClaudeOverload: "防止 Claude 超载",
  activeTabChange:
    "目前的标签已停用，因其他标签处于活动中。若要激活此标签，请按“确定”。",
  ...
  trash: "垃圾桶",
  trashDesc:
    "删除的角色将移至垃圾桶，可选择还原或永久删除。删除的角色将在 3 天后自动永久删除。",
  shareExport: "分享/导出",
  ...
  lowLevelAccessConfirm:
    "此内容使用低层级访问，可直接访问 AI 模型和你的存储数据。你确定要导入吗？",
  triggerLowLevelOnly:
    "此触发仅适用于低层级访问，需在角色或模块的高级设置中激活低层级访问。",
  truthy: "真值",
  ...
  convertionStep1:
    "选择与提示词相关的文件（支持 Context、Instruct 及 Sampler JSON）",
  customCSS: "自定义 CSS",
  ...
  childLoreDesc:
    "这是角色世界书的副本，保持“始终激活”，直到在原始版本中删除或手动停用。",
  ...
  pluginRisksInuserFriendlyDesc: {
    ...
    thisOutsideClass:
      "该插件在类上下文之外使用 'this' 关键字，这可能导致意外行为或安全问题。",
    errorInVerification: "插件验证过程中出现错误，表明其功能可能存在问题。",
    storageAccess:
      "该插件尝试访问存储，这可能导致数据泄露或未经授权的数据操作。",
  },
  ...
  mainDomAccessConsent:
    "插件 {} 正在请求访问主文档，这可能会泄露敏感信息。是否允许？",
  getFullDatabaseConsent:
    "插件 {} 正在请求访问完整数据库，这可能会泄露敏感信息。是否允许？",
  ...
  recentSessions: "最近对话列表",
  noRecentSessions: "暂无最近对话",
  searchSessions: "搜索对话...",
  noMatchingSessions: "没有找到匹配的对话",
