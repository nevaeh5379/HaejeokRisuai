...
export function preLoadCheck(){
    ...
    if (typeof window !== 'undefined') {
        const flushStores = () => {
            void import("./ts/stores/domain/settingsStore.svelte").then(({ settingsStore }) => settingsStore.flush())
            void import("./ts/stores/domain/characterStore.svelte").then(({ characterStore }) => characterStore.flush())
        }
