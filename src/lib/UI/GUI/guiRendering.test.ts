...
describe("GUI rendering without a browser session", () => {
  it("renders button primitives with snippet content, variants, and disabled behavior", async () => {
    ...
    const iconButton = icon.getBySelector<HTMLButtonElement>(
      "button#gui-render-icon-button",
    );
    ...
    expect(iconButton.textContent?.replace(/\s+/g, " ").trim()).toBe(
      "i Details",
    );
  ...
  it("renders text, password, and number inputs with values, attributes, and callbacks", async () => {
    ...
    const textInput = text.getBySelector<HTMLInputElement>(
      "input#gui-render-text",
    );
    const passwordInput = password.getBySelector<HTMLInputElement>(
      "input#gui-render-secret",
    );
    const numberInput = number.getBySelector<HTMLInputElement>(
      "input#gui-render-number",
    );
  ...
  it("renders checkbox and optional input states without app bootstrapping", async () => {
    ...
    const checkboxInput = checkbox.getBySelector<HTMLInputElement>(
      'input[type="checkbox"]',
    );
    expect(checkbox.target.querySelector("label")?.textContent).toContain(
      "Render checkbox",
    );
    ...
    const defaultInput =
      optionalText.getBySelector<HTMLInputElement>('input[type="text"]');
    expect(defaultInput.value).toBe("Using default");
    expect(defaultInput.disabled).toBe(true);

    const optionalNumberInput = optionalNumber.getBySelector<HTMLInputElement>(
      'input[type="number"]',
    );
    expect(optionalNumberInput.value).toBe("12");
    expect(
      optionalNumber.target.firstElementChild?.classList.contains("mb-4"),
    ).toBe(true);

    const booleanButtons = Array.from(
      optionalBoolean.target.querySelectorAll<HTMLButtonElement>("button"),
    );
    expect(booleanButtons.map((button) => button.textContent?.trim())).toEqual([
      "True",
      "False",
    ]);
  ...
  it("renders select, option, and segmented selection controls", async () => {
    ...
    const buttons = Array.from(
      segmented.target.querySelectorAll<HTMLButtonElement>(
        "[data-segment-btn]",
      ),
    );
    ...
    expect(
      segmented.target.querySelector(".segmented-indicator"),
    ).not.toBeNull();
  ...
  it("renders slider semantics and supports keyboard changes", async () => {
    ...
    const sliderElement =
      slider.getBySelector<HTMLDivElement>('[role="slider"]');
    ...
    const disableToggle = disableableSlider.getBySelector<HTMLInputElement>(
      'input[type="checkbox"]',
    );
    const disabledSliderElement =
      disableableSlider.getBySelector<HTMLDivElement>('[role="slider"]');
