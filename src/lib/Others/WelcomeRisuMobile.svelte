...
<div class="relative w-full h-full min-h-[100dvh] bg-bgcolor text-textcolor flex flex-col justify-between p-3.5 selection:bg-blue-600 selection:text-white overflow-x-hidden overflow-y-auto">
  {#if isTauriMacOS}
    <div
      class="absolute top-0 left-0 right-1 h-5 z-40"
        ...
        <div class="flex flex-col gap-3 w-full shrink-0 pb-1">
          ...
          {#if !isNodeServer}
            {#if showStorageForm}
              <div class="flex flex-col gap-3" in:fade={{ duration: 150 }}>
                ...
                <form
                  ...
                  onsubmit={(event) => { event.preventDefault(); void connectRemoteStorage(); }}
                >
                  <label class="block">
                    ...
                    <input
                      ...
                      autocomplete="url"
                      class="mt-1 w-full px-3 py-2.5 rounded-xl bg-darkbutton/40 border border-borderc text-textcolor text-xs focus:border-blue-500 outline-none transition-colors"
                    />
                  </label>
                  <label class="block">
                  ...
                  <button
                    type="submit"
                    class="w-full py-2.5 rounded-xl bg-selected font-bold text-xs text-textcolor disabled:opacity-50 flex items-center justify-center gap-1.5 transition-colors"
            ...
            {:else if storageMode === 'remote'}
              <div class="flex items-center justify-center gap-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-2.5 text-xs font-semibold text-emerald-300">
            ...
            {:else}
              <button
                ...
                class="w-full flex items-center justify-center gap-2 rounded-2xl border border-borderc/40 bg-darkbutton/30 active:bg-darkbutton/70 px-3 py-3 text-sm font-bold text-textcolor transition-colors"
              >
