      ...
      <div class="flex-1 flex flex-col justify-between py-2 sm:py-4 gap-4" in:fade={{ duration: 150 }}>
        ...
        <div class="flex flex-col gap-3 w-full shrink-0 pb-1">
          ...
          {#if !isNodeServer}
            ...
            {:else}
              <div class="flex items-center justify-between gap-2.5 rounded-2xl border border-borderc/40 bg-darkbg px-3 py-2.5 shadow-sm">
                <span class="flex items-center gap-2 min-w-0 text-[11px] text-textcolor2">
                ...
                {:else}
                  <button
                    type="button"
                    class="px-3 py-2 rounded-xl bg-darkbutton active:bg-selected border border-borderc/60 text-textcolor text-xs font-semibold transition-colors flex items-center gap-1.5 shrink-0"
                    onclick={() => { showStorageForm = true; storageError = ''; }}
                  >
                    <Server class="w-3 h-3 text-textcolor2" />
                    <span>{l.setup?.storageRemoteLabel || '서버 연결'}</span>
                  </button>
                {/if}
              </div>
            {/if}
