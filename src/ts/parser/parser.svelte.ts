...
const mdHighlight = markdownit({
  highlight: function (str, lang) {
    if (lang) {
      return (
        `<pre-hljs-placeholder lang="${lang}">` +
        str +
        "</pre-hljs-placeholder>"
      );
    }
    return "";
  },
...
DOMPurify.addHook("uponSanitizeElement", (node: HTMLElement, data) => {
  ...
  if (data.tagName === "img") {
    ...
    if (settingsStore.state?.hideAllImages) {
      ...
      if (
        src &&
        !src.startsWith("data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP")
      ) {
...
function renderMarkdown(md: markdownit, data: string) {
  ...
  let text = risuUnescape(
    md.render(data.replace(/“|”/g, '"').replace(/‘|’/g, "'")),
  );
...
async function renderHighlightableMarkdown(data: string) {
  ...
  for (const placeholder of highlightPlaceholders) {
    try {
      ...
      let languageModule: typeof import("highlight.js/lib/languages/*") | null =
        null;
      let fileExt = "";
...
async function parseAdditionalAssets(
  ...
  const assetWidthString =
    (settingsStore.state.assetWidth && settingsStore.state.assetWidth !== -1) ||
    settingsStore.state.assetWidth === 0
      ? `max-width:${settingsStore.state.assetWidth}rem;`
      : "";
  ...
  data = await replaceAsync(
    data,
    assetRegex,
    async (full: string, type: string, name: string) => {
...
async function parseInlayAssets(data: string) {
  ...
  if (inlayMatch) {
    for (const inlay of inlayMatch) {
      ...
      let prefix =
        inlayType !== "inlay" ? `<div class="risu-inlay-image">` : "";
      ...
      switch (asset?.type) {
        ...
        case "video": {
          ...
          data = data.replace(
            inlay,
            `${prefix}<video controls><source src="${url}" type="${videoMime}"></video>${postfix}`,
          );
          break;
        }
        case "audio": {
          const audioMime = asset?.name
            ? getMimeType(asset.name)
            : "audio/mpeg";
          data = data.replace(
            inlay,
            `${prefix}<audio controls><source src="${url}" type="${audioMime}"></audio>${postfix}`,
          );
          break;
        }
      }
...
export function addMetadataToElement(data: string, modelShortName: string) {
  ...
  console.log(
    ...
    Math.ceil(encodedMetaCode.length / 32),
    "<p> tags to store",
  );
        ...
        if (slt) {
          const parser = cssSelectorParser((root) => {
            root.walkClasses((classes) => {
              if (
                classes.type === "class" &&
                !classes.value.startsWith("x-risu-")
              ) {
...
const dateTimeFormat = (main: string, time = 0) => {
  ...
  return main
    ...
    .replace(
      /dddd/g,
      Intl.DateTimeFormat("en", { weekday: "long" }).format(date),
    )
    .replace(
      /ddd/g,
      Intl.DateTimeFormat("en", { weekday: "short" }).format(date),
    )
...
function blockStartMatcher(
  ...
  if (p1.startsWith("#when")) {
    if (p1.startsWith("#when ")) {
      ...
      const state = statement[1];
      return {
        type: state === "true" || state === "1" ? "newif" : "newif-falsy",
      };
  ...
  if (aChara) {
    if (typeof aChara !== "string" && aChara.type === "group") {
      if (aChara.chats[aChara.chatPage].message.length > 0) {
        const gc = findCharacterbyId(
          aChara.chats[aChara.chatPage].message.at(-1).saying ?? "",
        );
        ...
        if (parentStyle) {
          for (let i = 0; i < parentStyle.length; i++) {
            span.style.setProperty(
              parentStyle[i],
              parentStyle.getPropertyValue(parentStyle[i]),
            );
          }
        }
