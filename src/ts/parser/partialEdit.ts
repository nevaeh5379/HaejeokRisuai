...
export interface RangeResult {
  ...
  method:
    "exact" | "anchor" | "fuzzy" | "fuzzy+eol" | "fuzzy+eol+snap" | "bigram";
  confidence: number;
}
 ...
 */
function transformCBSPatterns(s: string): {
  processed: string;
  positionMap: number[];
} {
  let processed = "";
...
export function findAllOriginalRangesFromText(
  ...
  function deduplicateByLine(
    matches: RangeResultWithContext[],
  ): RangeResultWithContext[] {
  ...
  if (plN.length >= N * 2) {
    ...
    while (
      ...
      (headPos = mdN.indexOf(head, headPos)) >= 0
    ) {
      ...
      if (tailPos >= 0) {
        ...
        const confidence = Math.min(
          0.95,
          0.7 + (matchedLen / plN.length) * 0.25,
        );
  ...
  if (plN.length <= FUZZY_MAX) {
    ...
    for (const [candPos, candDist] of Array.from(uniqueCandidates.entries())) {
      ...
      const refineEnd = Math.min(
        mdN.length - plN.length,
        candPos + Math.floor(step / 2),
      );
