...
describe("#if_pure", () => {
  test('renders when "1" or "true"', () => {
    ...
    fc.assert(
      fc.property(
        ...
        fc.stringMatching(/^ +[^#:{}\r\n]*$/),
        (truthy, tail) => {
          expect(quickParse(`#if_pure ${truthy}${tail}`, "CBS")).toBe(
            `0 CBS 9`,
          );
        },
      ),
  ...
  test("can be nested", () => {
    ...
    expect(quickParse("#if_pure 1", template("#if_pure 0", indentedBody))).toBe(
      `0 0  9 9`,
    );
    expect(quickParse("#if_pure 0", template("#if_pure 1", indentedBody))).toBe(
      `0  9`,
    );
    expect(quickParse("#if_pure 0", template("#if_pure 0", indentedBody))).toBe(
      `0  9`,
    );
  });
});
...
describe("#when", () => {
  ...
  test("can be nested", () => {
    expect(quickParse("#when 1", template("#when 1", "CBS"))).toBe(
      `0 0 CBS 9 9`,
    );
  ...
  describe("Operators: logical", () => {
    ...
    test.skip("Lower precedence than other operators", () => {
      ...
      expect(quickParse("#when::3::tis::3::or::7::tis::7", "CBS")).toBe(
        `0 CBS 9`,
      );
    });
  });
  ...
  describe("Operators: whitespaces", () => {
    test("::keep preserves all whitespaces", () => {
      expect(quickParse("#when::keep::1", indentedBody)).toBe(
        `0 ${indentedBody} 9`,
      );
    });

    test("::legacy trims start of the block, end of the block, and start of each line", () => {
      expect(quickParse("#when::legacy::1", indentedBody)).toBe(
        `0 C  \nB  \nS 9`,
      );
    });
  });
    ...
    test("::var, toggle", () => {
      ...
      fc.assert(
        fc.property(
          validCBSArgProp.filter((s) => s !== "1" && s !== "true"),
          (anythingElse) => {
            expect(quickParse(`#when::var::${anythingElse}`, "CBS")).toBe(
              `0  9`,
            );
            expect(quickParse(`#when::toggle::${anythingElse}`, "CBS")).toBe(
              `0  9`,
            );
          },
        ),
    ...
    test("::vis, visnot, tis, tisnot", () => {
      fc.assert(
        fc.property(validCBSArgProp, validCBSArgProp, (a, b) => {
          ...
          expect(quickParse(`#when::${a}::visnot::${b}`, "CBS")).toBe(
            `0 CBS 9`,
          );
          expect(quickParse(`#when::${a}::tisnot::${a}`, "CBS")).toBe(`0  9`);
          expect(quickParse(`#when::${a}::tisnot::${b}`, "CBS")).toBe(
            `0 CBS 9`,
          );
        }),
      );
  ...
  describe("else", () => {
    ...
    test("with ::keep", () => {
      ...
      expect(
        quickParse("#when::keep::1", `${indentedBody}{{:else}}${revBody}`),
      ).toBe(`0 ${indentedBody.replace(/\n$/, "")} 9`);
      // FIXME: Unexpected line break removal after the {{:else}}
      expect(
        quickParse("#when::keep::0", `${indentedBody}{{:else}}${revBody}`),
      ).toBe(`0 ${revBody.replace(/^\n/, "")} 9`);
    });
