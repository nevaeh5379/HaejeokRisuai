...
const assetSaveMock = vi.hoisted(() => {
    ...
    return {
        state,
        saveAsset: vi.fn(async (_data: Uint8Array, _customId: string, fileName: string) => {
            state.active += 1
...
describe('CharXImporter', () => {
    ...
    it('saves more than ten assets without exceeding the concurrency limit', async () => {
        const assetNames = Array.from({ length: 12 }, (_, index) =>
            `assets/asset-${index}.${index % 2 === 0 ? 'png' : 'webp'}`
        )
        const archive = zipSync({
            'card.json': new TextEncoder().encode('{"name":"test"}'),
            ...Object.fromEntries(assetNames.map((name, index) => [
                name,
        ...
        expect(importer.assets).toEqual(Object.fromEntries(
            assetNames.map((name) => [name, `saved/${name}`]),
        ))
    }, 2_000)

    it('streams Node server assets in bounded bulk-write batches', async () => {
        const assetNames = Array.from({ length: 130 }, (_, index) =>
            `assets/asset-${index}.${index % 2 === 0 ? 'png' : 'webp'}`
        )
        const archive = zipSync({
            'card.json': new TextEncoder().encode('{"name":"test"}'),
