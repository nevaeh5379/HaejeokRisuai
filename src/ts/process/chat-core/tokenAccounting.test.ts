...
describe("shared chat token accounting", () => {
  it("batches text counting and applies message, name, and thought overhead", async () => {
    ...
    const counter = vi.fn(async (texts: string[]) => texts.map((text) => text.length));
  ...
  it("does not count names or thoughts when disabled", async () => {
    const chats: OpenAIChat[] = [{ role: "user", content: "abc", name: "Ignored", thoughts: ["x"] }];
    const result = await countChatTokensDetailed(chats, async (texts) => texts.map(() => 2), {
      ...baseOptions,
      useName: false,
      countThoughts: false,
    });
  ...
  it("uses the additional-token fallback when image input is unsupported", () => {
    expect(calculateMultimodalTokenCost({ type: "image", base64: "" }, {
      ...baseOptions,
  ...
  it("uses the legacy fixed low-quality vision cost", () => {
    expect(calculateMultimodalTokenCost({ type: "image", base64: "", width: 4000, height: 4000 }, {
      ...baseOptions,
      visionQuality: "low",
    })).toBe(87);
  });
  ...
  it("preserves the existing high-quality image scaling formula", () => {
    expect(calculateMultimodalTokenCost({ type: "image", base64: "", width: 2048, height: 1024 }, baseOptions)).toBe(104);
    expect(calculateMultimodalTokenCost({ type: "image", base64: "", width: 2048, height: 2048 }, baseOptions)).toBe(96);
  });

  it("rejects malformed batch counter results", async () => {
    await expect(countChatTokensDetailed(
