...
function convertOldToNewHypaV2Data(
  ...
  for (let i = 0; i < oldMainChunks.length; i++) {
    ...
    const previousMainChunkTarget = previousMainChunk
      ? previousMainChunk.targetId
      : null;
    ...
    const matchingOldChunks = oldChunks.filter(
      (oldChunk) => oldChunk.targetId === targetId,
    );
...
function cleanInvalidChunks(chats: OpenAIChat[], data: HypaV2Data): void {
  ...
  const validMainChunkIds = new Set(
    data.mainChunks.map((mainChunk) => mainChunk.id),
  );
...
export async function hypaMemoryV2(
  ...
  while (currentTokens > maxContextTokens) {
    ...
    if (!summaryData.success) {
      ...
      if (summarizationFailures >= maxSummarizationFailures) {
        console.error(
          "[HypaV2] Summarization failed multiple times. Aborting...",
        );
        return {
          currentTokens: currentTokens,
          chats: chats,
    ...
    console.log(
      "[HypaV2] tokens after summarization deduction:",
      currentTokens,
    );
  }
  ...
  while (
    ...
    if (tokenized > allocatedTokens - mainPromptTokens - chunkResultTokens)
      break;
