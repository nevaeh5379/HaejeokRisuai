...
export function stringlizeChatOba(
  ...
  for (const form of formated) {
    ...
    if (form.role === "user") {
      ...
      prefix = appendWhitespace(
        suggesting ? userPrefix : assistantPrefix,
        seperator,
      );
      name ??= `${characterName}`;
      name += ": ";
...
export function stringlizeAINChat(
  ...
  for (const form of formated) {
    console.log(form);
    if (
      (form.memo && form.memo.startsWith("newChat")) ||
...
function extractAINOutputStrings(inputString: string, characters: string[]) {
  ...
  while (remainingString.length > 0) {
    ...
    if (characterIndex == -1) {
      results.push({
        content: remainingString.trim(),
        character: "[narrator]",
      });
      break;
    } else {
      let endQuoteIndex = remainingString.indexOf(
        "」",
      ...
      if (endQuoteIndex == -1) {
        results.push({
          ...
          content: remainingString
            .substring(characterIndex + character.length + 1)
