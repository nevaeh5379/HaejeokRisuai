...
export class GraphMemClient extends MCPClientLike {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            search_depth: {
              type: "number",
              description:
                "The depth of connections to explore in the graph. default is 2.",
            },
          },
          required: ["query"],
        },
  ...
  private async handleWriteMemory(args: any): Promise<RPCToolCallContent[]> {
    ...
    return [
      { type: "text", text: `Memory entry "${name}" written successfully.` },
    ];
  }
  ...
  private async handleReadMemory(args: any): Promise<RPCToolCallContent[]> {
    ...
    if (!Array.isArray(query) || query.length === 0) {
      return [
        { type: "text", text: `Query must be a non-empty array of strings.` },
      ];
    }
