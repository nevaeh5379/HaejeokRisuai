...
export class CharacterHandler extends MCPToolHandler {
  ...
  getTools(): MCPTool[] {
    return [
      ...
      {
        description:
          "Get lorebooks with specific names from a Risuai character.",
        inputSchema: {
          properties: {
            id: {
              description:
        ...
        inputSchema: {
          properties: {
            alwaysActive: {
              default: false,
              description:
                "If true, the lorebook is always active regardless of keywords.",
              type: "boolean",
            },
            content: {
      ...
      },
      {
        ...
        inputSchema: {
          properties: {
            ...
            flag: {
              description:
                'Regex flags (e.g., "g", "i", "m") used when "ableFlag" is true.',
              type: "string",
            },
            id: {
              description:
  ...
  async handle(
    ...
    switch (toolName) {
      ...
      case "risu-list-character-lorebooks":
        return await this.getCharacterLorebooks(
          args.id,
          args.count,
          args.offset,
        );
      ...
      case "risu-delete-character-additional-assets":
        return await this.deleteCharacterAdditionalAssets(
          args.id,
          args.assetName,
        );
  ...
  async getCharacterLorebook(
    ...
    const entries = char.globalLore.filter((entry) => {
      const displayName =
        entry.comment || "Unnamed " + pickHashRand(5515, entry.content);
  ...
  async getCharacterRegexScripts(id: string): Promise<RPCToolCallContent[]> {
    ...
    const organized = (char.customscript || []).map((script) => {
      return {
        comment:
          script.comment ||
          "Unnamed " + pickHashRand(5515, script.in + script.out),
        in: script.in,
        out: script.out,
        type: script.type,
        flag: script.flag,
  ...
  async deleteCharacterRegexScripts(
    id: string,
    name: string,
  ): Promise<RPCToolCallContent[]> {
    const char: character | groupChat = getCharacter(id);
  ...
  async getCharacterAdditionalAssets(
    id: string,
  ): Promise<RPCToolCallContent[]> {
    const char: character | groupChat = getCharacter(id);
    if (!char) {
  ...
  async deleteCharacterAdditionalAssets(
    ...
    const assetIndex = char.additionalAssets.findIndex((asset) => {
      const displayName =
        asset[0] || "Unnamed " + pickHashRand(5515, asset[1] + asset[2]);
