...
test("lists installed modules with pagination", async () => {
  ...
  expect(
    await instance.handle("risu-list-modules", { count: 3, offset: 3 }),
  ).toMatchSnapshot();
  expect(
  ...
  await fc.assert(
    fc.asyncProperty(
      ...
      async (fieldsArg, targetIndex) => {
        ...
        expect(
          await instance.handle("risu-get-module-info", {
            fields,
            id: target.id,
          }),
        ).toEqual(makeToolResponse(expected));
      },
    ),
  );
});
...
test("lists lorebooks of a module with pagination", async () => {
  ...
  expect(
    await instance.handle("risu-list-module-lorebooks", {
      count: 3,
      offset: 3,
      id: "A",
    }),
...
test("retrieves a module Lua script", async () => {
  ...
  ).toEqual(makeToolResponse('print("hello")'));
});
...
test("errs if module not found", async () => {
  ...
  const errors = await Promise.all(
    subjects.map((tool) => instance.handle(tool, { id: "zzz" })),
  );
  ...
  expect(
    errors
      ...
      .every(
        (response) =>
          (response as RPCToolCallTextContent).text ===
          "Error: Module with ID zzz not found.",
      ),
  ).toBe(true);
});
