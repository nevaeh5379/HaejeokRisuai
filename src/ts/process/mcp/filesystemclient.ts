...
export class FileSystemClient extends MCPClientLike {
  ...
  constructor() {
    ...
    this.serverInfo.instructions =
      "FProvides file system access using the File System Access API";
  }
  ...
  private async initializeDirectory(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `FileSystemClient: Selected directory: ${this.directoryHandle.name}`,
      );
    } catch (error) {
      throw new Error(
        "Directory selection cancelled or failed. Please try again.",
      );
    }
  }
    ...
    return [
      {
        name: "fs_read_file",
        description:
          "Read contents of a file (supports text files, pdf, images)",
        inputSchema: {
          type: "object",
          properties: {
            path: {
        ...
        inputSchema: {
          ...
          properties: {
            path: {
              type: "string",
              description:
                "Path to the directory relative to selected directory (empty for root)",
            },
          },
          required: [],
        },
        ...
        inputSchema: {
          ...
          properties: {
            path: {
              type: "string",
              description:
                "Path to the directory to create relative to selected directory",
            },
          },
          required: ["path"],
        },
        ...
        inputSchema: {
          ...
          properties: {
            path: {
              type: "string",
              description:
                "Path to the file to delete relative to selected directory",
            },
          },
          required: ["path"],
        },
        ...
        inputSchema: {
          ...
          properties: {
            pattern: {
              type: "string",
              description:
                "Search pattern (supports glob patterns like *.js, test*)",
            },
            content: {
              type: "string",
        ...
        inputSchema: {
          ...
          properties: {
            ...
            byContent: {
              type: "boolean",
              description:
                "Compare by content hash (default: false, compares by name)",
            },
          },
          required: [],
        },
    ...
    try {
      switch (toolName) {
        ...
        case "fs_find_duplicates":
          return await this.findDuplicates(
            args.path || "",
            args.byContent || false,
          );
        case "fs_tree_view":
  ...
  private async readFileAsPDF(
    ...
    const { convertPdfToImages } =
      await import("src/ts/process/dynamicutils/pdf.js");
  ...
  private async searchFiles(args: {
    ...
    const searchDir = searchPath
      ? await this.getDirectoryHandle(searchPath)
      : this.directoryHandle;
