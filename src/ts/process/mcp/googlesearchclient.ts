...
export class GoogleSearchClient extends MCPClientLike {
  ...
  private async initializeCredentials(): Promise<void> {
    const storedCredentials =
      await this.credentialsStorage.getItem<GoogleSearchCredentials>(
        "google-search-creds",
      );
    ...
    if (
      ...
      storedCredentials.searchEngineId
    ) {
    ...
    const searchEngineId = await alertInput(
      "Please enter your Google Custom Search Engine ID:",
    );
        ...
        inputSchema: {
          ...
          properties: {
            ...
            num: {
              type: "integer",
              description:
                "Number of search results to return (1-10, default: 10)",
              minimum: 1,
              maximum: 10,
              default: 10,
            },
            start: {
              type: "integer",
              description:
                "The index of the first result to return (default: 1)",
              minimum: 1,
              default: 1,
            },
            dateRestrict: {
            ...
            siteSearch: {
              type: "string",
              description:
                "Restrict search to a specific site (e.g., 'reddit.com')",
            },
            fileType: {
              type: "string",
              description:
                "Restrict search to specific file types (e.g., 'pdf', 'doc')",
            },
            language: {
              type: "string",
              description:
        ...
        inputSchema: {
          ...
          properties: {
            ...
            num: {
              type: "integer",
              description:
                "Number of image results to return (1-10, default: 10)",
              minimum: 1,
              maximum: 10,
              default: 10,
            },
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return [
          {
            type: "text",
            text: `Image search API error (${response.status}): ${errorText}`,
          },
        ];
      }
      ...
      if (!data.items || data.items.length === 0) {
        return [
          {
            type: "text",
            text: `No image results found for query: "${query}"`,
          },
        ];
      }
