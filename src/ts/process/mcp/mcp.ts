...
async function initializeMCPsUnlocked(
  ...
  for (const mcp of mcpUrls) {
    if (!MCPs[mcp]) {
      ...
      if (mcp.startsWith("stdio:")) {
        ...
        try {
          ...
          if (MCPData.url) {
            ...
            const args: string[] = Array.isArray(MCPData.args)
              ? MCPData.args
              : [MCPData.args];
            ...
            const listeners = new Set<
              (message: JsonRPC) => void | Promise<void>
            >();
      ...
      const registerRefresh: typeof MCPClient.prototype.registerRefreshToken = (
        arg,
      ) => {
      ...
      const getRefresh: typeof MCPClient.prototype.getRefreshToken =
        async () => {
          return settingsStore.state.authRefreshes?.find(
            (refresh) => refresh.url === mcp,
          );
        };
...
export async function importMCPModule() {
  const x = await alertInput(
    "Please enter the URL of the MCP module to import:",
    [
...
export async function decodeToolCall(
  text: string,
): Promise<toolCallData | undefined> {
  text = text.trim();
