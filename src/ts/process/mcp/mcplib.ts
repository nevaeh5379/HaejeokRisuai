...
export class MCPClient {
  ...
  customTransport?: {
    ...
    removeListener: (
      callback: (message: JsonRPC) => void | Promise<void>,
    ) => void;
  };
      ...
      if (this.sseEndpoint && !options.notifications) {
        responsePromise = new Promise<RPCRequestResult>((resolve) => {
          const sseListener = (event: CustomEvent<SseEventDetail>) => {
            if (event.detail.mcpClientObjectId !== this.mcpClientObjectId)
              return;
  ...
  async handshake() {
    ...
    if (http.status === 404) {
      console.warn(
        "MCP: Streamed transport not supported, falling back to SSE",
      );
      ...
      const connectionResult = await new Promise<RPCRequestResult>(
        (resolve) => {
          const sseListener = (event: CustomEvent<SseEventDetail>) => {
            if (event.detail.mcpClientObjectId !== this.mcpClientObjectId)
              return;
  ...
  async oauthLogin() {
    ...
    const registerResponse = await fetchNative(
      discoveryURLS.registration_endpoint,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const authHelperResponse = await fetchNative(
      "https://account.sionyw.com/oauthhelper/api",
      {
        method: "POST",
        body: JSON.stringify({
          code: code,
