...
import {
  ...
  setCurrentChat,
  type Message,
  type StreamingDisplayOptimizationMode,
...
async function runChatOutputListeners(
  char: any,
  chat: any,
...
export async function sendChat(
  ...
  if (nowChatroom.type === "group") {
    if (chatProcessIndex === -1) {
      const charNames = nowChatroom.characters.map(
        (v) => findCharacterbyIdwithCache(v).name,
      );
      ...
      let order = nowChatroom.characters
        .map((v, i) => {
          return {
            id: v,
            talkness: nowChatroom.characterActive[i]
              ? nowChatroom.characterTalks[i]
              : -1,
            index: i,
          };
        })
  ...
  if (!currentChar.utilityBot && !promptTemplate) {
    const mainp =
      currentChar.systemPrompt?.replaceAll(
        "{{original}}",
  ...
  if (currentChat.note) {
    ...
    unformated.authorNote.push({
      role: "system",
      content: risuChatParser(getAuthorNoteDefaultText(), {
        chara: currentChar,
      }),
    });
  }
  ...
  if (
    ...
    !(
      usingPromptTemplate &&
      settingsStore.state.promptSettings.customChainOfThought
    )
  ) {
  ...
  function getDescriptionPrompts(role?: "user" | "bot" | "system") {
    const pmt = [
      ...
      ...(baseDescriptionPrompt
        ? [safeStructuredClone(baseDescriptionPrompt)]
        : []),
    ...
    for (const card of template) {
      switch (card.type) {
        ...
        case "chat": {
          ...
          let end =
            card.rangeEnd === "end" ? unformated.chats.length : card.rangeEnd;
          if (start === -1000) {
            start = 0;
  ...
  for (const msg of ms) {
    ...
    if (msg.role === "char") {
      ...
      const inlayMatch = formatedChat.match(
        /{{(inlay|inlayed|inlayeddata)::(.+?)}}/g,
      );
      if (inlayMatch) {
    ...
    if (
      ...
      (nowChatroom.type === "group" &&
        settingsStore.state.groupOtherBotRole === "assistant") ||
    ...
    let thoughts: string[] = [];
    const maxThoughtDepth =
      settingsStore.state.promptSettings?.maxThoughtTagDepth ?? -1;
    formatedChat = formatedChat.replace(
    ...
    formatedChat = formatedChat.replace(
      ...
      (match, p1) => {
        ...
        const assets = (currentChar.additionalAssets ?? []).concat(
          moduleAssets,
        );
  ...
  if (
    ...
    if (settingsStore.state.hanuraiEnable) {
      ...
      characterStore.characters[selectedChar].chats[selectedChat].hypaV2Data =
        currentChat.hypaV2Data;

      currentChat = characterStore.characters[selectedChar].chats[selectedChat];
      console.log(
        "[Expected to be updated] chat's HypaV2Data: ",
        currentChat.hypaV2Data,
      );
      ...
      if (sp.error) {
        ...
        if (sp.memory) {
          ...
          characterStore.characters[selectedChar].chats[
            selectedChat
      ...
      characterStore.characters[selectedChar].chats[selectedChat].hypaV3Data =
        currentChat.hypaV3Data;

      currentChat = characterStore.characters[selectedChar].chats[selectedChat];
      console.log(
        "[Expected to be updated] chat's HypaV3Data: ",
        currentChat.hypaV3Data,
      );
    } else {
  ...
  let biases: [string, number][] = settingsStore.state.bias
    .concat(currentChar.bias)
    .map((v) => {
      return [
        risuChatParser(
          v[0]
  ...
  unformated.chats = chats
    ...
    .filter((v) => {
      return (
        v.content.trim() !== "" || (v.multimodals && v.multimodals.length > 0)
      );
    });
  ...
  if (
    ...
    (settingsStore.state.aiModel.startsWith("claude") ||
      ...
      settingsStore.state.aiModel.startsWith("openrouter") ||
      settingsStore.state.aiModel.startsWith("reverse_proxy"))
  ) {
    unformated.postEverything.push({
      role: "system",
  ...
  function pushPrompts(cha: OpenAIChat[]) {
    for (const chat of cha) {
      if (
        !chat.content.trim() &&
        !(chat.multimodals && chat.multimodals.length > 0)
      ) {
        continue;
      }
      if (!(
    ...
    for (const card of template) {
      switch (card.type) {
        ...
        case "memory": {
          ...
          if (card.innerFormat && pmt.length > 0) {
            for (let i = 0; i < pmt.length; i++) {
              pmt[i].content = risuChatParser(card.innerFormat, {
                chara: currentChar,
              }).replace("{{slot}}", pmt[i].content);
  ...
  if (req.type === "fail") {
    ...
    characterStore.characters[selectedChar].chats[selectedChat].isStreaming =
      true;
    characterStore.characters[selectedChar].chats[
      selectedChat
    ].activeStreamingDisplayOptimizationMode = performanceMode;
    ...
    const coalesceStreamingDisplay =
      performanceMode === "balanced" || performanceMode === "strong";
      ...
      streamingFlushPromise = (async () => {
        do {
          ...
          if (deferStreamingPostProcessing) {
            characterStore.characters[selectedChar].chats[selectedChat].message[
              msgIndex
            ].data = reformatContent(prefix + nextResult);
            characterStore.characters[selectedChar].reloadKeys += 1;
            continue;
          }
          let result2 = await processScriptFull(
            nowChatroom,
            reformatContent(prefix + nextResult),
            "editoutput",
            msgIndex,
          );
          characterStore.characters[selectedChar].chats[selectedChat].message[
    ...
    currentChat = characterStore.characters[selectedChar].chats[selectedChat];
    const inlayMessageIndex = findMessageIndexByChatId(
      currentChat,
      outputMessageId,
    );
    ...
    if (outputMessage) {
      ...
      if (inlayr.promise) {
        ...
        currentChat =
          characterStore.characters[selectedChar].chats[selectedChat];
        const asyncInlayMessageIndex = findMessageIndexByChatId(
          currentChat,
          outputMessageId,
        );
    ...
    currentChat = characterStore.characters[selectedChar].chats[selectedChat];
    const listenerMessageIndex = findMessageIndexByChatId(
      currentChat,
      outputMessageId,
    );
    ...
    currentChat = characterStore.characters[selectedChar].chats[selectedChat];
    if (outputMessageId) {
      outputMessageIndex = findMessageIndexByChatId(
        currentChat,
        outputMessageId,
      );
      await runChatOutputListeners(
  ...
  if (!currentChar.inlayViewScreen) {
    if (
      ...
      if (settingsStore.state.emotionProcesser === "embedding") {
        ...
        let searched = (await hypaProcesser.similaritySearchScored(result)).map(
          (v) => {
  ...
  if (currentFinishedChat?.id) {
    ...
    if (msgsToCommit.length > 0) {
      void messageStore
        .commitMessages(currentFinishedChat.id, msgsToCommit)
        .catch((err) => {
          console.error(
            "[requestProcess] Failed to commit chat messages:",
            err,
          );
        });
    }
