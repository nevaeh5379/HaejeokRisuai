...
export function detectPromptJSONType(text: string) {
  ...
  try {
    ...
    if (
      ...
      notNull(parsed.temp) &&
      notNull(parsed.rep_pen) &&
      notNull(parsed.min_length)
    ) {
      return "PARAMETERS";
    } else if (notNull(parsed.story_string) && notNull(parsed.chat_start)) {
      return "STCONTEXT";
    } else if (
      notNull(parsed.input_sequence) &&
      notNull(parsed.output_sequence)
    ) {
      return "STINST";
    }
  } catch (e) {}
...
export async function promptConvertion(
  ...
  if (type === "STCHAT") {
    ...
    alertNormal(
      "Preset converted successfully. You can find it in bot setting presets",
    );
    return;
  }
