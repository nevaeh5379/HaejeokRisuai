...
async function runPostprocessorPipeline(
  ...
  for (const hook of hooks) {
    ...
    try {
      ...
      console.error(
        "[TTS postprocessor] threw, continuing with next hook:",
        err,
      );
      continue;
    }
    ...
    if (!result) continue;
    if (result.skip)
      return { audio: currentAudio, mimeType: currentMime, skip: true };
...
export async function sayTTS(character: character, text: string) {
  try {
    ...
    const beforeResult = await runHookPipeline<
      BeforeTTSContext,
      BeforeTTSResult
    >(getTTSPreprocessors(), {
      text,
    ...
    switch (character.ttsMode) {
      ...
      case "openai": {
        const cfg = character.oaiTTSConfig?.enabled
          ? character.oaiTTSConfig
          : null;
        const baseURL = (
          cfg?.baseURL?.trim() || "https://api.openai.com/v1"
        ).replace(/\/+$/, "");
        ...
        if (res.ok) {
          ...
          if (dat.error && dat.error.message) {
            ...
            alertError(
              language.errors.httpError + `${Buffer.from(res.data).toString()}`,
            );
          }
        }
        break;
      ...
      case "huggingface": {
        while (true) {
          if (character.hfTTS.language !== "en") {
            text = await runTranslator(
              text,
              false,
              "en",
              character.hfTTS.language,
            );
          }
      ...
      case "gptsovits": {
        const audio: Uint8Array = await loadAsset(
        ...
        const body = {
          ...
          ref_free:
            character.gptSoVitsConfig.use_long_audio ||
            !character.gptSoVitsConfig.use_prompt,
        };
        ...
        if (character.gptSoVitsConfig.use_auto_path) {
          console.log("auto");
          const path = await globalFetch(
            `${character.gptSoVitsConfig.url}/get_path`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
        ...
        const response = await globalFetch(
          `${character.gptSoVitsConfig.url}/tts`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
