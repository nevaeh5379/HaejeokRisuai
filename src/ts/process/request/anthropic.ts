...
export async function requestClaude(
  ...
  if (aiModel === "reverse_proxy" && db.autofillRequestUrl) {
    if (replacerURL.endsWith("v1")) {
      ...
      replacerURL.endsWith("messages") || replacerURL.endsWith("messages/")
    )) {
  ...
  if (bedrock && aiModel !== "reverse_proxy") {
    ...
    if (additionalParams.length > 0) {
      params = applyAdditionalParameters(
        params,
        bedrockHeaders,
        additionalParams,
      );
        ...
        while (true) {
          try {
            ...
            if (Date.now() - batchStartTime > BATCH_TIMEOUT) {
              controller.error(
                new Error("Claude batch request timed out after 24 hours"),
              );
              return;
            }
            ...
            if (statusRes.status !== 200) {
              const statusResponseText = await textifyReadableStream(
                statusRes.body,
              );
              addFetchLog({
                body: batchRequestBody,
            ...
            if (batchRes.status !== 200) {
              const resultResponseText = await textifyReadableStream(
                batchRes.body,
              );
              addFetchLog({
                body: batchRequestBody,
            ...
            const batchTextData = batchResultRawText
              .split("\n")
              .filter((v) => v.trim() !== "")
              .map((v) => {
                try {
            ...
            console.error(
              "Error while waiting for Claude batch results:",
              error,
            );
          }
        }
...
async function requestClaudeHTTP(
  ...
  if (!res.ok) {
    ...
    return {
      type: "fail",
      result: stringlified,
      failByServerError: stringlified
        ?.toLocaleLowerCase()
        ?.includes("overload"),
    };
  }
