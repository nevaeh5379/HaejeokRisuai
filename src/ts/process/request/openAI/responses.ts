...
async function decodeRememberedToolCallsForResponses(
  text: string,
  ...
  for (const segment of segments) {
    ...
    if (currentContent.trim()) {
      items.push({
        content: [
          { type: "output_text", text: currentContent, annotations: [] },
        ],
        role: "assistant",
        status: "completed",
        type: "message",
...
async function buildResponseInputItems(
    ...
    switch (content.role) {
      ...
      case "assistant": {
        if (
          ...
          content.content.includes("<tool_call>")
        ) {
          items.push(
            ...(await decodeRememberedToolCallsForResponses(content.content)),
          );
          break;
        }
      ...
      case "system": {
        const role =
          content.role === "system" && developerRole
            ? "developer"
            : content.role;
        ...
        for (const multimodal of multimodals) {
          if (multimodal.type === "image") {
            item.content.push({
              type: "input_image",
              detail: (db.gptVisionQuality ?? "auto") as
                "high" | "low" | "auto",
...
function getResponsesRequestURL(arg: RequestDataArgumentExtended): {
  requestURL: string;
...
async function buildResponsesBody(
  ...
  const responseParameters = arg.modelInfo.parameters.filter(
    (p) =>
      ["temperature", "top_p", "reasoning_effort", "verbosity"].includes(p) ||
      isReasoningCapabilityParameter(p),
  );
...
function extractResponsesFunctionCalls(data: any): ResponseFunctionCallItem[] {
  return (data?.output ?? [])
    ...
    .filter(
      (item: any): item is ResponseFunctionCallItem =>
        item?.type === "function_call",
    );
}

async function appendResponsesToolOutputs(
  body: any,
  ...
  return [
    assistantText && !db.simplifiedToolUse ? assistantText : "",
    callCodes.join("\n\n"),
  ]
    .filter(Boolean)
    .join("\n\n");
...
async function requestHTTPResponsesAPI(
  requestURL: string,
  body: any,
  ...
  if (data?.status === "incomplete") {
    ...
    const reason = data?.incomplete_details?.reason
      ? `Incomplete response: ${data.incomplete_details.reason}`
      : "Incomplete response";
  ...
  if (calls.length > 0) {
    ...
    const prefix = await appendResponsesToolOutputs(
      body,
      calls,
      arg,
      assistantText,
    ...
    do {
      attempt++;
      resRec = await requestHTTPResponsesAPI(
        requestURL,
        body,
        headers,
        arg,
...
function getResponsesTranStream(
  ...
  const emit = (
    controller: TransformStreamDefaultController<StreamResponseChunk>,
  ) => {
    let result = text;
    if (reasoning) {
      result = `<Thoughts>\n\n${reasoning}\n\n</Thoughts>\n${result}`;
  ...
  const applyEvent = (event: any) => {
    ...
    if (
      ...
      const key =
        event.call_id ?? event.item_id ?? event.output_index?.toString() ?? "0";
      calls[key] ??= {
        type: "function_call",
...
function wrapResponsesToolStream(
    ...
    async start(controller) {
      ...
      while (true) {
        ...
        const calls = Object.values(
          JSON.parse(lastValue?.["__tool_calls"] || "{}") || {},
        ) as ResponseFunctionCallItem[];
        if (calls.length === 0) {
          controller.close();
          return;
        }
        ...
        const callPrefix = await appendResponsesToolOutputs(
          body,
          calls,
          arg,
          lastValue?.["0"] ?? "",
...
export async function requestOpenAIResponseAPI(
  ...
  if (arg.useStreaming) {
    ...
    return {
      type: "streaming",
      result: wrapResponsesToolStream(
        transtream.readable,
        body,
        headers,
        requestURL,
  ...
  return requestHTTPResponsesAPI(
    requestURL,
    body,
    headers,
    arg,
