...
describe("local network request ownership", () => {
  ...
  it("reads the active preset using the real guarded stores", () => {
    expect(getLocalNetworkRequestOptions("http://192.168.1.2:8080/v1", undefined, true))
      .toEqual({ networkRoute: "local_network", requestTimeoutMs: 12000 });
    presetStore.state.localNetworkTimeoutSec = 30;
    expect(getLocalNetworkRequestOptions("http://localhost:8080/v1", undefined, true))
  ...
  it("uses the timeout fallback only for streaming local requests", () => {
    ...
    expect(getLocalNetworkRequestOptions("http://localhost", undefined, true))
      .toEqual({ networkRoute: "local_network", requestTimeoutMs: 600000 });
    expect(getLocalNetworkRequestOptions("http://localhost"))
      .toEqual({ networkRoute: "local_network", requestTimeoutMs: undefined });
  });
});
