...
const baseArg = (overrides: Record<string, any> = {}) =>
  ({
    ...
    formated: [
      ...
      {
        role: "user",
        content: "Also read file.",
        multimodals: [
          { type: "audio", base64: "data:application/pdf;base64,def" },
        ],
      },
    ],
    maxTokens: 321,
...
describe("OpenAI Responses API helpers", () => {
  ...
  it("builds a Responses request body for text, developer role, multimodal input, tools, and model parameters", async () => {
    ...
    const body = await __testResponsesAPI.buildResponsesBody(
      baseArg({
    ...
    expect(body.input[2]).toMatchObject({
      ...
      content: [
        {
          type: "output_text",
          text: "Previous assistant prefill",
          annotations: [],
        },
      ],
    });
    ...
    expect(sourceMessages[1]).toMatchObject({
      role: "user",
      content: "Describe this.",
      multimodals: [{ type: "image", base64: "data:image/png;base64,abc" }],
    });
  ...
  it("lets reverse proxy additional params override Responses reasoning summaries", async () => {
    mocks.db.additionalParams = [["reasoning.summary", "detailed"]];
  ...
  it("uses OpenAI defaults and an externally clean non-streaming body", async () => {
    ...
    expect(mocks.globalFetch.mock.calls[0][1].body).not.toHaveProperty(
      "__lastOutput",
    );
  });
  ...
  it("preserves reasoning summaries and refusal-only responses", () => {
    expect(
      __testResponsesAPI.extractResponsesText(
        {
          output: [
        ...
        output: [
          {
            ...
            content: [
              {
                type: "reasoning_text",
                text: "Hmm, the user just greeted me...",
              },
            ],
            summary: [],
            format: "unknown",
    ...
    const text = __testResponsesAPI.extractResponsesText(
      {
        output: [
          {
            id: "rs_0d1786ac1d609512016a07343fdca8819ca2d651a07a09a86d",
            type: "reasoning",
            summary: [],
          },
          {
            type: "message",
            content: [{ type: "output_text", text: "Only final answer." }],
          },
        ],
      },
      baseArg(),
  ...
  it("sanitizes streaming Responses tool continuation input for store false", async () => {
    vi.mocked(callTool).mockResolvedValueOnce([
      { type: "text", text: "stream tool result" },
    ] as any);
    ...
    await collectStream(
      result.result as ReadableStream<Record<string, string>>,
    );
    expect(vi.mocked(fetchNative)).toHaveBeenCalledTimes(2);
    const followupBody = JSON.parse(
      vi.mocked(fetchNative).mock.calls[1][1].body as string,
    );
    expect(JSON.stringify(followupBody.input)).not.toContain(
      "rs_stream_ignored",
    );
    expect(followupBody.input).toEqual(
      expect.arrayContaining([
    ...
    expect(
      followupBody.input.find((item: any) => item.type === "function_call"),
  ...
  it("parses split CRLF SSE chunks, final unterminated events, text deltas, and function call deltas", async () => {
    ...
    await writer.write(
      encoder.encode(
        'data: {"type":"response.output_item.done","item":{"type":"function_call","call_id":"call_1","name":"lookup","arguments":"{\\"q\\":\\"x\\"}","status":"completed"}}',
      ),
    );
    await writer.close();
  ...
  it("does not duplicate text from a completed streaming event", async () => {
    ...
    await writer.write(
      encoder.encode(
        'data: {"type":"response.completed","response":{"output_text":"Hello","output":[{"type":"message","content":[{"type":"output_text","text":"Hello"}]}]}}\n\n',
      ),
    );
    await writer.close();
  ...
  it("uses completed streaming reasoning content without duplicating final output text", async () => {
    ...
    await writer.write(
      encoder.encode(
        'data: {"type":"response.completed","response":{"output":[{"type":"reasoning","content":[{"type":"reasoning_text","text":"Reasoned once"}],"summary":[]},{"type":"message","content":[{"type":"output_text","text":"Hello"}]}]}}\n\n',
      ),
    );
    await writer.close();
