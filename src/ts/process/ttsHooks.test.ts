...
const makeCtx = (
  overrides: Partial<BeforeTTSContext> = {},
): BeforeTTSContext => ({
  text: "hello",
  ttsMode: "openai",
  characterId: "char-1",
...
describe("runHookPipeline", () => {
  it("returns the original ctx when no hooks are registered", async () => {
    const ctx = makeCtx();
    const result = await runHookPipeline<BeforeTTSContext, BeforeTTSResult>(
      [],
      ctx,
      1000,
    );
  ...
  it("applies a single hook that transforms text", async () => {
    const hook: TTSHookFn<BeforeTTSContext, BeforeTTSResult> = async (c) => ({
      text: c.text + "!",
    });
  ...
  it("chains two hooks — second sees the first's output", async () => {
    const a: TTSHookFn<BeforeTTSContext, BeforeTTSResult> = async (c) => ({
      text: c.text + " A",
    });
    const b: TTSHookFn<BeforeTTSContext, BeforeTTSResult> = async (c) => ({
      text: c.text + " B",
    });
  ...
  it("short-circuits on skip=true and does not call later hooks", async () => {
    ...
    const b = vi.fn<TTSHookFn<BeforeTTSContext, BeforeTTSResult>>(
      async (c) => ({ text: c.text + " B" }),
    );
    const result = await runHookPipeline([a, b], makeCtx(), 1000);
  ...
  it("treats an undefined return as a no-op", async () => {
    const noop: TTSHookFn<BeforeTTSContext, BeforeTTSResult> = async () =>
      undefined;
  ...
  it("ignores undefined fields in a result (does not overwrite ctx with undefined)", async () => {
    const hook: TTSHookFn<BeforeTTSContext, BeforeTTSResult> = async () => ({
      text: undefined,
    });
    const result = await runHookPipeline(
      [hook],
      makeCtx({ text: "keep-me" }),
      1000,
    );
  ...
  it("accepts synchronous (non-Promise) returns", async () => {
    const hook: TTSHookFn<BeforeTTSContext, BeforeTTSResult> = (c) => ({
      text: c.text + " S",
    });
    const result = await runHookPipeline([hook], makeCtx(), 1000);
