...
describe("coldstorageData", () => {
  ...
  it("maps unavailable cold storage keys to affected character names", () => {
    ...
    const affected = getColdStorageAffectedCharacters(
      ...
      [characterKey, storedChatKey, activeChatKey, unknownKey],
    );
  ...
  it("rewrites character cold storage asset references without mutating the original payload", () => {
    const payload = {
      character: {
        ...
        additionalAssets: [["prop", "assets/local-prop.png"]],
      },
    };
    ...
    expect(rewritten.character.emotionImages[0][1]).toBe(
      "assets/account-neutral.png",
    );
    expect(rewritten.character.emotionImages[1][1]).toBe(
      "assets/local-happy.png",
    );
    expect(rewritten.character.additionalAssets[0][1]).toBe(
      "assets/account-prop.png",
    );
    expect(payload.character.image).toBe("assets/local-main.png");
    expect(payload.character.emotionImages[0][1]).toBe(
      "assets/local-neutral.png",
    );
    expect(payload.character.additionalAssets[0][1]).toBe(
      "assets/local-prop.png",
    );
  });
  ...
  it("leaves non-character cold storage payloads unchanged", () => {
    ...
    expect(
      replaceColdStoragePayloadResources(payload, {
