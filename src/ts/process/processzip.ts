...
export async function processZip(dataArray: Uint8Array): Promise<string> {
  ...
  const imageFile = Object.keys(unzipped).find((fileName) =>
...
export class CharXWriter {
  ...
  constructor(
    private writer:
      LocalWriter | WritableStreamDefaultWriter<Uint8Array> | VirtualWriter,
  ) {
...
export class CharXImporter {
  ...
  #flushNodeAssets() {
    ...
    this.nodeAssetSaveChain = this.nodeAssetSaveChain.then(
      saveBatch,
      saveBatch,
    );
  }
