...
export function addLorebook(type: number) {
  ...
  if (type === 0) {
    characterStore.characters[selectedID].globalLore.push({
      key: "",
      comment: `New Lore ${characterStore.characters[selectedID].globalLore.length + 1}`,
      content: "",
      mode: "normal",
      insertorder: 100,
      alwaysActive: false,
    ...
    const page = characterStore.characters[selectedID].chatPage;
    characterStore.characters[selectedID].chats[page].localLore.push({
      key: "",
      comment: `New Lore ${characterStore.characters[selectedID].chats[page].localLore.length + 1}`,
      content: "",
      mode: "normal",
      insertorder: 100,
      alwaysActive: false,
...
export async function loadLoreBookV3Prompt(
  ...
  const loreToken =
    char.loreSettings?.tokenBudget ?? settingsStore.state.loreBookToken;
  ...
  while (matching) {
    ...
    for (let i = 0; i < fullLore.length; i++) {
      ...
      const content = CCardLib.decorator.parse(
        fullLore[i].content,
        (name, arg) => {
          switch (name) {
            case "end": {
            ...
            case "disable_ui_prompt": {
              if (
                ["post_history_instructions", "system_prompt"].includes(arg[0])
              ) {
  ...
  for (const lore of loreinjectionLores) {
    ...
    if (foundLoreIndex !== -1) {
      ...
      switch (lore.inject.operation) {
        ...
        case "replace": {
          foundLore.prompt = foundLore.prompt.replace(
            lore.inject.param,
            lore.prompt,
          );
          break;
...
export async function importLoreBook(mode: "global" | "local" | "sglobal") {
  const selectedID = get(selectedCharID);
  const page =
    mode === "sglobal" ? -1 : characterStore.characters[selectedID].chatPage;
  let lore =
    mode === "global"
      ? characterStore.characters[selectedID].globalLore
      : characterStore.characters[selectedID].chats[page].localLore;
  ...
  try {
    ...
    if (mode === "global") {
      characterStore.characters[selectedID].globalLore = lore;
    } else {
      characterStore.characters[selectedID].chats[page].localLore = lore;
    }
  } catch (error) {
    alertError(error);
  }
...
export function convertExternalLorebook(entries: {
  [key: string]: CCLorebook;
}) {
  let lore: loreBook[] = [];
  for (const key in entries) {
    ...
    lore.push({
      ...
      comment:
        currentLore.comment ||
        currentLore.name ||
        currentLore.displayName ||
        "",
      content:
        currentLore.content || currentLore.entry || currentLore.text || "",
      mode: "normal",
      alwaysActive:
        currentLore.constant ?? currentLore.forceActivation ?? false,
      secondkey: currentLore.secondary_keys
        ? currentLore.secondary_keys.join(", ")
        : "",
