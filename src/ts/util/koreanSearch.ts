...
function charToPattern(ch: string, isLastChar: boolean): string {
  ...
  const choseongIndex = CHOSEONG_LIST.indexOf(
    ch as (typeof CHOSEONG_LIST)[number],
  );
  if (choseongIndex !== -1) {
    const startCode =
      HANGUL_BASE + choseongIndex * JUNGSEONG_COUNT * JONGSEONG_COUNT;
  ...
  if (code >= HANGUL_BASE && code <= HANGUL_END) {
    ...
    if (isLastChar) {
      ...
      if (extraIndices && extraIndices.length > 1) {
        ...
        const chars = extraIndices.map((idx) =>
          String.fromCharCode(baseSyllableWithoutBatchim + idx),
        );
        return `(?:${chars.join("|")})`;
      }
    }
 ...
 */
export function buildKoreanSearchRegex(
  query: string,
  flags = "i",
): RegExp | null {
...
export function matchKoreanText(
  ...
  if (regex && regex.test(targetNorm)) {
    const isStart = regex.test(
      targetNorm.slice(0, Math.max(queryLower.length + 2, 4)),
    );
  ...
  if (/[가-힣]/.test(queryNorm)) {
    try {
      const romanizedQuery = romanize(queryNorm)
        .toLowerCase()
        .replace(/\s+/g, "");
      if (romanizedQuery) {
...
export function matchCharacterKorean(
  ...
  return {
    ...
    score: finalScore,
    isQwertyConverted:
      nameResult.isQwertyConverted || creatorResult.isQwertyConverted,
