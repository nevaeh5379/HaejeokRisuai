...
describe("koreanSearch unit tests", () => {
  describe("matchKoreanText", () => {
    ...
    it("handles pure choseong search (초성 검색)", () => {
      ...
      expect(matchKoreanText("블루 아카이브", "ㅂㄹㅇㅋㅇㅂ").matched).toBe(
        true,
      );
    ...
    it("handles whitespace insensitivity (공백 무시)", () => {
      ...
      expect(matchKoreanText("블루 아카이브", "블루아카이브").matched).toBe(
        true,
      );
      expect(matchKoreanText("블루 아카이브", "ㅂㄹ ㅇㅋㅇㅂ").matched).toBe(
        true,
      );
    });
