...
describe("isLocalNetworkUrl", () => {
  it("accepts local URLs", () => {
    ...
    expect(isLocalNetworkUrl("http://my-box.local/v1/chat/completions")).toBe(
      true,
    );
    expect(isLocalNetworkUrl("http://litellm:4000/v1/chat/completions")).toBe(
      true,
    );
  });
  ...
  it("rejects public URLs", () => {
    expect(
      isLocalNetworkUrl("https://api.openai.com/v1/chat/completions"),
    ).toBe(false);
