...
export async function fetchProtectedResource(
  ...
  //for production, settingsStore.state.account.token should be removed entirely
  if (localStorage.getItem("ignoreRisuAuth") !== "true") {
    risuAuth = settingsStore.state?.account?.token;
...
async function fetchProtectedResourceSPA(
  ...
  if (!tokenInitalized || arg.forceRefresh) {
    ...
    const oauthData: SionywOauthData = JSON.parse(
      new TextDecoder().decode(oauthDataText),
    );
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        let refreshedTokens: client.TokenEndpointResponse =
