...
export function registerCBS(arg: CBSRegisterArg) {
  ...
  const resolveSpeaker = (matcherArg: matcherArg) => {
    if (matcherArg.chara && typeof matcherArg.chara !== "string") {
      return matcherArg.chara;
    }
    return resolveRoom(matcherArg).room;
  };
  ...
  registerFunction({
    ...
    callback: (str, matcherArg, args, vars) => {
      ...
      if (matcherArg.chara) {
        if (typeof matcherArg.chara === "string") return matcherArg.chara;
        return matcherArg.chara.nickname || matcherArg.chara.name;
      }
      const currentChar = resolveRoom(matcherArg).room;
  ...
  registerFunction({
    ...
    callback: (str, matcherArg, args, vars) => {
      const { room: selchar, chat } = resolveRoom(matcherArg);
      let pointer =
        matcherArg.chatID !== -1
          ? matcherArg.chatID - 1
  ...
  registerFunction({
    name: "personality",
    callback: (str, matcherArg, args, vars) => {
      const achara = resolveSpeaker(matcherArg);
  ...
  registerFunction({
    ...
    callback: (str, matcherArg, args, vars) => {
      const achara = resolveSpeaker(matcherArg);
      const { room, chat } = resolveRoom(matcherArg);
      const characterLore =
  ...
  registerFunction({
    name: "moduleenabled",
    callback: (str, matcherArg, args, vars) => {
      const modules = getModules(resolveRoom(matcherArg).room);
  ...
  registerFunction({
    ...
    callback: (str, matcherArg, args, vars) => {
      const module = getModules(resolveRoom(matcherArg).room)?.find((f) => {
  ...
  registerFunction({
    name: "hash",
    callback: (str, matcherArg, args, vars) => {
      return (pickHashRand(0, args[0]) * 10000000 + 1)
        .toFixed(0)
        .padStart(7, "0");
    },
    alias: [],
  ...
  const randomPickImpl = (
    ...
    const index = matcherArg.tokenizeAccurate
      ? 0
      : Math.floor(rand * arr.length);
  ...
  registerFunction({
    name: "pick",
    callback: (str, matcherArg, args, vars) => {
      const { room: selchar, chat: selChat } = resolveRoom(matcherArg);
  ...
  registerFunction({
    ...
    callback: (str, matcherArg, args, vars) => {
      ...
      if (args.length > 1) {
        return (
          `<pre-hljs-placeholder lang="${args[0]}">` +
          code +
  ...
  registerFunction({
    ...
    alias: [],
    description:
      "A comment CBS for commenting out code.\n\nUsage:: {{// this is a comment}}",
  });
