...
export async function getOpenRouterProviders(): Promise<
...
export async function getOpenRouterModels(): Promise<OpenRouterModelInfo[]> {
  try {
    ...
    return aim.data
      .map((model: any) => {
        const price =
          (Number(model.pricing.prompt) * 3 +
            Number(model.pricing.completion)) /
          4;
        const priceDisplay =
          price > 0 ? `$${(price * 1000).toFixed(5)}/1k` : "Free";
        const legacyName =
          price > 0
        ...
        const provider =
          colonIdx !== -1
            ...
            : model.id.split("/")[0];
        const cleanName =
          colonIdx !== -1 ? model.name.slice(colonIdx + 1).trim() : model.name;
        ...
        return {
          ...
          internalReasoningPrice1M: toPrice1M(
