...
export function applyRulesToString(
  text: string,
...
function getAspectRatioCssValue(
  aspectRatio?: string,
  customHeight?: number,
): string {
  switch (aspectRatio) {
...
function isCustomCaption(alt: string | null | undefined): boolean {
  ...
  if (lower.startsWith("/sw/") || lower.includes("/") || lower.includes("\\"))
    return false;
...
function applyGalleryFrame(
  ...
  styleImg(
    image,
    { borderRadius: "0", display: "block", width: "100%", maxWidth: "100%" },
    !isCropped,
    {
...
function applyModernFrame(
  ...
  const frame = div({
    ...
    boxSizing: "border-box",
    boxShadow:
      "0 12px 32px rgba(0,0,0,0.1), 0 2px 6px rgba(0,0,0,0.05), inset 0 0 0 1px #fcfcfc",
  });
  styleImg(
    image,
    {
...
function processImages(
  ...
  const scale =
    options.imageScale && options.imageScale !== 100 ? options.imageScale : 100;
  ...
  for (const el of images) {
    ...
    if (isCropped) {
      ...
      const aspect = getAspectRatioCssValue(
        options.imageCropAspectRatio,
        options.imageCropHeight,
      );
      ...
      const hPos =
        options.imageCropHAlign !== undefined ? options.imageCropHAlign : 50;
      const vPos =
        options.imageCropVAlign !== undefined ? options.imageCropVAlign : 50;
 ...
 */
export async function processMessageHtml(
  options: ProcessMessageOptions,
): Promise<string> {
  const { html, embedImages, color, replacementRules, allowHtmlRendering } =
    options;
