...
export async function loadAllCharSettings(): Promise<CharacterSettingsMap> {
    try {
        return parseRecord(await settingsStore.loadPluginCustomStorageKey(CHAR_SETTINGS_KEY)) as CharacterSettingsMap
    } catch (e) {
...
export async function loadCharSettings(charId: string): Promise<Partial<LogExporterSettings>> {
    const all = await loadAllCharSettings()
    return parseRecord(all[charId]) as Partial<LogExporterSettings>
}

export async function saveCharSettings(charId: string, settings: Partial<LogExporterSettings>): Promise<void> {
    try {
        const all = await loadAllCharSettings()
...
export async function loadGlobalExporterSettings(): Promise<GlobalExporterSettings> {
    try {
        return parseRecord(await settingsStore.loadPluginCustomStorageKey(GLOBAL_SETTINGS_KEY)) as GlobalExporterSettings
    } catch (e) {
...
export async function saveGlobalExporterSettings(patch: Partial<GlobalExporterSettings>): Promise<void> {
    try {
