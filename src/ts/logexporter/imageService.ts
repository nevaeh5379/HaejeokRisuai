...
async function captureElementToBlob(
    ...
    const blob = await toBlob(element, { ...options, type: mime, quality: format === 'jpeg' ? 1 : undefined })
...
async function forEachSection(
    ...
    for (let i = 0; i < numSections; i++) {
        ...
        try {
            ...
            const blob = await captureElementToBlob(wrapper, format === 'webp' ? 'png' : format, bgColor, resolution)
...
function createSectionWrapper(
    ...
    Object.assign(wrapper.style, {
        ...
        width: `${totalWidth}px`, height: `${sectionHeight}px`,
    ...
    try {
        ...
        try {
            ...
            if (isTooTall && settings.splitImage === 'chunk') {
                const blobs: Blob[] = []
                await forEachSection(element, finalMaxHeight, resolution, format, bgColor, async (b) => {
                    blobs.push(b)
                }, onProgressUpdate)
                ...
                await forEachSection(element, finalMaxHeight, resolution, format, bgColor, async (b) => {
                    onProgressUpdate?.({ message: `[섹션 ${sectionIndex + 1}/${numSections}] 파일 저장 중...` })
                    await downloadBlob(b, `${baseFilename}_part${++sectionIndex}.${format}`)
...
async function inlineRemainingImages(element: HTMLElement): Promise<void> {
    const { imageUrlToDataUrl, extractBackgroundImageUrl } = await import('./messageRenderer')
