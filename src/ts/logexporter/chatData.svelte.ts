 ...
 */
export async function collectLogData(options: CollectLogDataOptions = {}): Promise<LogExportData> {
    ...
    if (options.singleMessage !== undefined) {
        ...
        const end = options.endIndex !== undefined ? options.endIndex + 1 : rawMessages.length
    ...
    return { charInfo, messages, participants, characterId: char.chaId ?? '' }
}
