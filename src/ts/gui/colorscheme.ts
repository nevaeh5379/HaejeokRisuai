...
export const colorSchemeList = Object.keys(
  colorShemes,
) as (keyof typeof colorShemes)[];

export function changeColorScheme(colorScheme: string) {
  try {
    if (colorScheme === "custom") {
      settingsStore.state.colorScheme = safeStructuredClone(
        settingsStore.state.customColorScheme ?? defaultColorScheme,
      );
    } else {
      settingsStore.state.colorScheme = safeStructuredClone(
        colorShemes[colorScheme],
      );
    }
...
export function updateColorScheme() {
  try {
    ...
    document.documentElement.style.setProperty(
      ...
      colorScheme.textcolor2,
    );
    document.documentElement.style.setProperty(
      "--risu-theme-darkborderc",
      colorScheme.darkBorderc,
    );
    document.documentElement.style.setProperty(
...
export function updateTextThemeAndCSS() {
  ...
  switch (textTheme) {
    ...
    case "custom": {
      ...
      root.style.setProperty(
        "--FontColorQuote1",
        db.customTextTheme.FontColorQuote1 ?? "#8BE9FD",
      );
      root.style.setProperty(
        "--FontColorQuote2",
        db.customTextTheme.FontColorQuote2 ?? "#FFB86C",
      );
      break;
    }
  }
