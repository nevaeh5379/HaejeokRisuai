...
export const languageSettingsItems: SettingItem[] = [
  ...
  // Translator Specific Configurations
  {
    ...
    condition: (ctx) =>
      !!ctx.db.translator && ctx.db.translatorType === "deepl" && !isTauri,
  },

  {
    ...
    classes: "mt-4",
    condition: (ctx) =>
      !!ctx.db.translator && ctx.db.translatorType === "deepl",
  },
...

  {
    ...
    classes: "mt-4",
    condition: (ctx) =>
      !!ctx.db.translator && ctx.db.translatorType === "deeplX",
  },
...

  {
    ...
    classes: "mt-4",
    condition: (ctx) =>
      !!ctx.db.translator && ctx.db.translatorType === "google",
    options: {
      selectOptions: [
...

  {
    ...
    classes: "mt-4",
    condition: (ctx) =>
      !!ctx.db.translator && ctx.db.translatorType === "bergamot",
  },
      ...
      onClick: async () => {
        try {
          ...
          const confirmed = await alertConfirm(
            language.importTranslationCacheConfirm,
          );
          if (!confirmed) return;
          alertWait(language.loading);
          ...
          if (failed > 0) {
            ...
            alertNormal(
              language.importTranslationCacheSuccess.replace(
                "{0}",
                String(count),
              ),
            );
...

  {
    ...
    options: {
      onClick: async () => {
        try {
          const confirmed = await alertConfirm(
            language.clearTranslationCacheConfirm,
          );
          if (!confirmed) return;
          alertWait(language.loading);
          await clearLLMCache();
