...
import {
  registerMCPModule,
  unregisterMCPModule,
} from "src/ts/process/mcp/pluginmcp";
...
class SafeElement {
  ...
  public async addEventListener(
    ...
    if (allowedDocumentEventListeners.includes(type)) {
      ...
      throw new Error(
        `Event listener of type '${type}' is not allowed for security reasons.`,
      );
    }
  }
  ...
  public removeEventListener(
    ...
    if (listener) {
      ...
      const idx = documentEventListeners.findIndex(
...
class SafeDocument extends SafeElement {
  ...
  createElement(tagName: string): SafeElement {
    if (!tagWhitelist.includes(tagName.toLowerCase())) {
      console.warn(
        `Creation of <${tagName}> elements is restricted. Creating a <div> instead.`,
      );
      tagName = "div";
    }
  ...
  createAnchorElement(href: string): SafeElement {
    ...
    try {
      ...
      console.warn(
        `Invalid URL provided for anchor element: ${href}. Setting href to '#' instead.`,
      );
...
type SafeMutationCallback = (
  mutations: SafeClassArray<SafeMutationRecord>,
) => void;
...
class SafeMutationObserver {
  ...
  constructor(callback: SafeMutationCallback) {
    this.#observer = new MutationObserver((mutations) => {
      const safeMutations: SafeMutationRecordObject[] = mutations.map(
        (mutation) => {
      ...
      for (const record of safeMutations) {
        safeClassed.push(
          new SafeMutationRecord(record.type, record.target, record.addedNodes),
        );
      }
  ...
  observe(element: SafeElement, options: MutationObserverInit) {
    ...
    const rawElement = document.querySelector(
  ...
  return {
    ...
    addTTSPreprocessor: async (
      ...
      addPluginUnloadCallback(plugin.name, () =>
        unregisterTTSPreprocessor(func),
      );
    },
    addTTSPostprocessor: async (
      ...
      addPluginUnloadCallback(plugin.name, () =>
        unregisterTTSPostprocessor(func),
      );
    },
    ...
    setChatPanel: (
      ...
      const panel = {
        ...
        className:
          typeof options.className === "string"
            ? DOMPurify.sanitize(options.className, {
                ALLOWED_TAGS: [],
                ALLOWED_ATTR: [],
              })
            : undefined,
      ...
      addPluginUnloadCallback(plugin.name, () =>
        removePluginChatPanels(plugin.name),
      );
      return { id };
    },
...
export async function executePluginV3(plugin: RisuPlugin) {
  ...
  if (alreadyRunning) {
    console.log(
      `[RisuAI Plugin: ${plugin.name}] Plugin is already running. Skipping load.`,
    );
    return;
  }
