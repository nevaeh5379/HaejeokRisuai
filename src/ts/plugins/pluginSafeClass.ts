...
const restrictNodeList = <
  T extends Element,
  Q extends NodeListOf<T> | HTMLCollectionOf<T>,
>(
  nodeList: Q,
): Q => {
  return nodeList;
...
export const SafeDocument = {
  ...
  createElement: (tagName: string): HTMLElement => {
    ...
    if (!tagWhitelist.includes(tagName.toLowerCase())) {
      throw new Error(
        `Creation of <${tagName}> elements is not allowed in plugin context.`,
      );
    }
  ...
  createElementNS: (namespaceURI: string, qualifiedName: string): Element => {
    ...
    if (!tagWhitelist.includes(qualifiedName.toLowerCase())) {
      throw new Error(
        `Creation of <${qualifiedName}> elements is not allowed in plugin context.`,
      );
    }
  ...
  createAnchorElement: (href: string): HTMLAnchorElement => {
    ...
    try {
      ...
      throw new Error(
        `Invalid URL provided for anchor element in plugin context.`,
      );
    }
