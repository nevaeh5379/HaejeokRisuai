...
export async function importCharacterProcess<T extends boolean = false>(f: {
  ...
  if (
    typeof (parsed as CharacterCardV2Risu)?.data?.character_version === "number"
  ) {
...
function convertOffSpecCards(
  ...
  const charbook =
    charaData.spec_version === "2.0" ? charaData.data.character_book : null;
  ...
  return {
    ...
    creatorNotes: "",
    systemPrompt:
      (charaData.spec_version === "2.0" ? charaData.data.system_prompt : "") ??
      "",
    postHistoryInstructions:
      (charaData.spec_version === "2.0"
        ? charaData.data.post_history_instructions
        : "") ?? "",
    alternateGreetings: [],
    tags: [],
    creator: "",
...
async function importCharacterCardSpec<T extends boolean = false>(
    ...
    if (risuext.emotions) {
      for (let i = 0; i < risuext.emotions.length; i++) {
        ...
        if (risuext.emotions[i][1].startsWith("__asset:")) {
          ...
          if (!imgp) {
            throw new Error(
              "Error while importing, asset " + key + " not found",
            );
          }
    ...
    if (risuext.vits) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (risuext.vits[key].startsWith("__asset:")) {
          ...
          if (!imgp) {
            throw new Error(
              "Error while importing, asset " + rkey + " not found",
            );
          }
  ...
  let char: character = {
    ...
    extentions: ext ?? {},
    largePortrait:
      data?.extensions?.risuai?.largePortrait ?? !data?.extensions?.risuai,
    ...
    if (
      ...
      switch (extensions.selectiveLogic) {
        ...
        case 1: {
          book.selective = false;
          content =
            `@@exclude_keys_all ${book.secondary_keys.join(",")}\n` + content;
          break;
        }
        case 2: {
          book.selective = false;
...
function createBaseV2(char: character) {
  ...
  char.loreExt.risu_fullWordMatching =
    char.loreSettings?.fullWordMatching ?? false;
...
async function exportCharacterCardNodeStream(
  ...
  entries.push({
    name: "card.json",
    dataBase64: Buffer.from(JSON.stringify(card, null, 4), "utf-8").toString(
      "base64",
    ),
  });
  ...
  const completionBody = (await completed.json().catch(() => null)) as {
    status?: string;
    error?: string;
  } | null;
  ...
  try {
    ...
    if (spec === "v2") {
      ...
      if (
        ...
        for (let i = 0; i < card.data.extensions.risuai.emotions.length; i++) {
          alertStore.set({
            ...
            submsg: (
              (i / card.data.extensions.risuai.emotions.length) *
              100
            ).toFixed(2),
          });
          const key = card.data.extensions.risuai.emotions[i][1];
          const rData = await readImage(key);
          const b64encoded = Buffer.from(await compressImage(rData)).toString(
            "base64",
          );
          assetIndex++;
      ...
      if (
        ...
        for (
          let i = 0;
          i < card.data.extensions.risuai.additionalAssets.length;
          i++
        ) {
          alertStore.set({
            type: "progress",
          ...
          assetIndex++;
          card.data.extensions.risuai.additionalAssets[i][1] =
            `__asset:${assetIndex}`;
          await writer.write("chara-ext-asset_:" + assetIndex, b64encoded);
        }
      }
          ...
          if (type === "png") {
            ...
            if (imageType === "PNG" && writer instanceof CharXWriter) {
              ...
              if (Object.keys(metadatas).length > 0) {
                await writer.write(
                  metaPath,
                  Buffer.from(JSON.stringify(metadatas, null, 4)),
                  6,
                );
              } else {
                await writer.write(
      ...
      if (type === "charx" || type === "charxJpeg") {
        ...
        await writer.write(
          "card.json",
          Buffer.from(JSON.stringify(card, null, 4)),
        );
      } else {
        await writer.write(
          "ccv3",
...
export async function shareRisuHub2(
  ...
  try {
    ...
    const dat =
      Buffer.from(writer.buf.buffer).toString("base64") + "&" + "rt.png";
