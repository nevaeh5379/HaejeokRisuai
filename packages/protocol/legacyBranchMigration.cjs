...
function messageSignature(message) {
  ...
  return JSON.stringify(cloned, (_key, value) =>
...
function materializeLegacyTimeline(chat, branch) {
  ...
  const base = Number.isInteger(state.baseMessageIndex)
...
function buildLegacyBranchMigrationPlan(chat, idFactory) {
  const state = chat?.branchState;
  const sourceBranches = Array.isArray(state?.branches)
    ? state.branches.filter((branch) => branch?.id)
    : [];
  ...
  const root =
    ordered.find((branch) => branch.reason === "root") ||
    ordered.find(
      (branch) =>
  ...
  const activeBranchId = branchIds.has(state.activeBranchId)
