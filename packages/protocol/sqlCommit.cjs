...
const RESERVED_ROOT_SETTING_KEYS = Object.freeze([
  "botPresets",
  "botPresetsId",
]);
...
function createSqlCommitValidator({ PayloadError, maxIdLength = 4000 } = {}) {
  ...
  function validateEntityRows(value, field, ownerKey) {
    return validateRows(value, field, (row, index) => {
      ...
      if (ownerKey !== undefined) {
        assertId(row[ownerKey], `${rowField}.${ownerKey}`);
        result[ownerKey] = row[ownerKey];
      }
  ...
  function validateManifests(value, field, ownerKey) {
    return validateRows(value, field, (item, index) => {
      ...
      assertId(item[ownerKey], `${itemField}.${ownerKey}`);
  ...
  return function validateSqlCommit(payload) {
      ...
      rootUpserts = asArray(payload.root.upserts, "root.upserts").map(
        (item, index) => {
          ...
          if (RESERVED_ROOT_SETTING_KEYS.includes(item.key)) {
            throw new PayloadError(
              `${item.key} must be written through presets`,
            );
          }
    ...
    if (payload.presets !== undefined) {
      ...
      const upserts = asArray(payload.presets.upserts, "presets.upserts").map(
        (item, index) => {
          if (!item || typeof item !== "object" || Array.isArray(item)) {
            throw new PayloadError(
      ...
      const deletes = asArray(payload.presets.deletes, "presets.deletes").map(
        (id, index) => {
    ...
    if (payload.pluginStorage !== undefined) {
      if (
        !payload.pluginStorage ||
        typeof payload.pluginStorage !== "object" ||
        Array.isArray(payload.pluginStorage)
      ) {
