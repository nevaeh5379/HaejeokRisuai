...
import {
  ...
  type LocalBackupDatabasePreparationProgress,
  type PreparedLocalBackupDatabase,
} from "./importDatabase";
import { buildBackupImportPlan, type BackupImportPlan } from "./importPlan";
import {
...
export class LocalBackupImportService {
  ...
  private async readValidatedColdStorageEntry(
    entry: StagedBackupEntry,
  ): Promise<{ key: string; value: unknown }> {
    const key: string | null = getColdStorageBackupKey(entry.name);
    if (!key) {
  ...
  private async validateColdStorage(
    id: string,
    ...
    for (let index: number = 0; index < entries.length; index++) {
      const entry: StagedBackupEntry = entries[index];
      await this.readValidatedColdStorageEntry(entry);
  ...
  private validateAssets(
    ...
    for (let index: number = 0; index < entries.length; index++) {
      const entry: StagedBackupEntry = entries[index];
      normalizeBackupAssetPath(entry.name);
      this.update(id, "assets", index + 1, entries.length, entry.name);
    }
  }

  private async validateInlays(
    id: string,
    entries: readonly StagedBackupEntry[],
  ): Promise<void> {
    if (entries.length === 0) return;
    ...
    for (let index: number = 0; index < entries.length; index++) {
      ...
      const bytes: Uint8Array = new Uint8Array(
        await fs.readFile(entry.filePath),
      );
      decodeInlayAssetBackup(bytes);
  ...
  private async restoreColdStorage(
    ...
    for (let index: number = 0; index < entries.length; index++) {
      const entry: StagedBackupEntry = entries[index];
      const { key, value }: { key: string; value: unknown } =
        await this.readValidatedColdStorageEntry(entry);
  ...
  private async restoreAssets(
    ...
    for (let index: number = 0; index < entries.length; index++) {
      const entry: StagedBackupEntry = entries[index];
      const key: string = normalizeBackupAssetPath(entry.name);
  ...
  private async restoreStream(
    ...
    try {
      ...
      const plan: BackupImportPlan = buildBackupImportPlan(staged);

      // Prepare and validate the database before any adapter mutation.
      const prepared: PreparedLocalBackupDatabase =
        await prepareLocalBackupDatabaseImport(plan, {
          encodeRecord: (record: LegacyBackupSqlRecord): unknown =>
            this.adapter.encodeDatabaseRecord(record),
      ...
      // Prevalidate every non-database payload sequentially from the staged
      // files so no adapter write happens before all entries are known good.
      await this.validateColdStorage(id, plan.coldStorage);
      await this.validateAssets(id, plan.assets);
      await this.validateInlays(id, plan.inlays);
      ...
      const result: LocalBackupImportRestoreResult =
        await this.adapter.applyPreparedDatabase(
          prepared,
