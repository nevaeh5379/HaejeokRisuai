...
function materializeColdCharacterForCompatibility(
  source,
  values,
  options = {},
) {
...
function applyBranchScriptState(chat, branch, cloneValue) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(branch, "useLocallySetGlobalVariables")
  ) {
    if (branch.useLocallySetGlobalVariables == null)
      delete chat.useLocallySetGlobalVariables;
    else
      chat.useLocallySetGlobalVariables = branch.useLocallySetGlobalVariables;
  }
}

function makeStandaloneChat(
  source,
  messages,
  name,
  idFactory,
...
function expandChatBranchesForCompatibility(
  source,
...
function expandCharacterBranchesForCompatibility(
  ...
  const nextChats = [];
  let nextChatPage = 0;
  const sourceChats = Array.isArray(materialized.chats)
    ? materialized.chats
    : [];
